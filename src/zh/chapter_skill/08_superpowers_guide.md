# 9.8 Skills 圣经：Superpowers 工程实践指南
...
Superpowers 的每一个 Skill 本质上是一个 `SKILL.md` 文件。这个设计解决了一个核心矛盾：

![Superpowers：SKILL.md 按需加载 vs 传统做法](../svg/chapter_skill_08_skill_loading.svg)
...
> **用户指令**（CLAUDE.md、直接请求）→ 高于 → **Superpowers Skills** → 高于 → **默认系统提示**
...
![Superpowers Skill 调用决策流程](../svg/chapter_skill_08_skill_flow.svg)
...
> `Brainstorm` → `Write Plans` → `Setup Worktree` → `Subagent-Driven Dev`（或 `Execute Plans`）→ `Debug`（如需）→ `Verify` → `Code Review` → `Finish Branch`
...
![Superpowers Skills 五层架构](../svg/chapter_skill_08_five_layer.svg)
...
> **Brainstorming（发散）** → 思考过程、方案对比、设计规格 → **Writing Plans（收敛）** → 可执行的任务清单 + 测试计划
...
![Subagent-Driven Development 双阶段审查流程](../svg/chapter_skill_08_skill_flow.svg)
...
>
> 没有测试的 AI 代码生成 = 没有终止条件的递归——AI 可以无限生成“看起来合理”但实际错误的代码。测试是唯一客观的验收标准。
...
|---------|---------|---------|
| 模式 / 规则 | 具体经验 | 当前会话 |
| `semantic/patterns.json` | `episodic/YYYY-MM-DD-*.json` | `working/session.json` |
...
*本章完。返回：[第9章 Skill System](./README.md)*
