...
后面按"**定义 → 管理 → 调度 → 组装**"四步走，每一步解决一个独立问题，可以单独读懂再拼起来：

| 步骤 | 做什么 | 核心产出 | 关键点 |
|------|--------|---------|--------|
| 第一步 | 定义技能 | `SKILL.md`（数据/审查/报告三个技能） | Prompt-based 定义，声明式 |
| 第二步 | 实现技能管理器 | `skill_manager.py` | 按需加载 + 发现 + 调用 |
| 第三步 | Agent 核心逻辑 | `agent.py` | 根据任务自动选技能 |
| 第四步 | 主程序 | `main.py` | 把前三步串成可运行 CLI |
