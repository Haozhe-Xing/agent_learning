...
先看最基础的一层——`DataAnalyzer` 把原始数据变成描述性统计（总行数、列名、均值/方差等）。这是后续可视化和洞察的**地基**：没有统计，图表和 LLM 洞察都无从谈起。它刻意做得"纯函数化"（不调 LLM、不依赖外部服务），保证可测可复用。
...
有了统计结果，`ChartGenerator` 把它变成图表。关键设计是**图表类型由数据特征决定**（而非硬编码）——数值列画直方图、分类列画柱状图，让可视化适配数据，而不是反过来为每种图写死逻辑。
...
[24.4 报告生成与导出](./04_report_generation.md)
