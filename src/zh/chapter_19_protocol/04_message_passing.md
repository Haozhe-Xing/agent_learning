# 19.4 Agent 间的消息传递与状态共享

> **本节目标**：学习 Agent 间消息传递的实现方式，并通过 MCP 工具集成实战加深理解。

![Agent 间消息传递与状态共享架构](../svg/chapter_19_protocol_04_message_passing.svg)

---

## 生产级 MCP 工具服务器

Agent 之间的"消息传递"落到工程上，最常见的第一种形态是**通过 MCP 服务器暴露工具**——一个 Agent 把自己的能力封装成 MCP 工具，另一个 Agent 通过 MCP 客户端调用它。下面的代码是一个"生产级"示例：与玩具代码的区别在于它补齐了**错误处理、类型校验、日志**这三样生产必需的东西。
...
*下一节：[19.5 实战：基于 MCP 的工具集成](./05_practice_mcp_integration.md)*
