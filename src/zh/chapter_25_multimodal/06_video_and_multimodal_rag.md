...
![视频理解的三层能力（自下而上递进）](../svg/chapter_25_multimodal_06_three_levels.svg)
...
| 架构 | 流程 | 优点 | 缺点 | 适用 |
|------|------|------|------|------|
| **① 文本优先**（Text-first） | OCR/图像描述 → 纯文本 Embedding → 文本检索 | 复用现有 RAG 基础设施，成本低 | 丢失视觉信息（布局、颜色、空间关系） | 以文字为主的文档（合同、发票） |
| **② 多模态 Embedding** | 图像+文本 → 统一向量空间 → 跨模态检索 | 可以用文字检索图片、用图片检索文字 | 需要专门的跨模态 Embedding 模型 | 图文混排文档（PPT、论文、手册） |
| **③ 原生多模态**（Native） | 直接将图像送入多模态 LLM 理解 | 信息零损失，理解最准确 | 成本高、速度慢 | 图像理解质量要求极高的场景 |
