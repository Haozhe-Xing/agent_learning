...
如果你用的是 LangChain / LangGraph，思路相同：先把 `ClientSession` 打开并保活，再把 `session.call_tool` 包成工具注入 Agent，**Agent 运行期间不要关闭 session**。第 13 章的 LangGraph 实战与第 16 章的多 Agent 系统都可沿用这个约束。
