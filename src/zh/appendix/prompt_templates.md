...
|------|---------|---------|---------|
| **1. Agent 系统提示词** | | | |
| 1.1 | 通用助手 | 通用 Agent 系统提示词 | 第2章 |
| 1.2 | 工具使用型 Agent | 需要调用外部工具的 Agent | 第3章 |
| 1.3 | ReAct 推理型 Agent | 需要多步推理和工具交替的 Agent | 第5章 |
| **2. 任务型 Prompt** | | | |
| 2.1 | 文本摘要 | 对长文本生成摘要 | 第2章 |
| 2.2 | 代码审查 | 审查代码质量和安全性 | 第16章 |
| 2.3 | 数据分析 | 从数据中提取洞察 | 第24章 |
| **3. Few-shot 示例** | 少样本学习任务 | 第2章 |
| **4. 结构化输出** | | | |
| 4.1 | JSON 格式输出 | 需要结构化 JSON 返回 | 第8章 |
| 4.2 | Markdown 报告输出 | 生成格式化报告 | 第24章 |
| **5. 安全防护提示词** | Prompt 注入防御 | 第21章 |
| **6. Multi-Agent 协调** | | | |
| 6.1 | 任务分配（Orchestrator） | 多 Agent 任务编排 | 第18章 |
| 6.2 | 专家 Agent（Worker） | 专家角色定义 | 第18章 |
| 6.3 | Agent 间消息传递 | Agent 协作通信 | 第19章 |
| **7. RAG 增强** | | | |
| 7.1 | 带上下文的问答 | 基于检索结果回答 | 第6章 |
| 7.2 | 查询改写 | 优化搜索查询 | 第6章 |
| 7.3 | 答案评估 | 判断是否需要补充检索 | 第6章 |
| **8. 结构化推理** | | | |
| 8.1 | 链式思考（CoT） | 复杂问题分步推理 | 第2章 |
| 8.2 | 决策分析矩阵 | 多方案对比决策 | 第5章 |
| 8.3 | 假设验证 | 数据驱动的假设检验 | 第5章 |
| **9. 自我反思与修正** | | | |
| 9.1 | 回答后自检 | 输出质量自我校验 | 第5章 |
| 9.2 | 错误分析与重试 | 从错误中学习并修正 | 第5章 |
| 9.3 | 多角度反驳 | 辩证分析观点 | 第5章 |
| **10. Guardrails 防护** | | | |
| 10.1 | 输出格式强制 | 严格控制输出格式 | 第8章 |
| 10.2 | 有害内容过滤 | 安全内容审核 | 第21章 |
| 10.3 | 幻觉检测 | 检测 AI 编造的信息 | 第21章 |
| **11. 领域特化** | | | |
| 11.1 | 金融分析 | 财务数据分析 | 第24章 |
| 11.2 | 教育辅导 | 教学引导 | — |
| 11.3 | 医疗健康咨询 | 健康信息科普 | — |
| **12. 多轮对话管理** | | | |
| 12.1 | 上下文保持 | 长对话上下文管理 | 第7章 |
...
> 💡 **使用建议**：以上模板是起点而非终点。在实际项目中，建议根据具体场景不断迭代优化。好的 Prompt 是"炼"出来的——通过大量测试用例验证效果，逐步调整措辞和结构。推荐使用第 20 章介绍的评估方法，对 Prompt 效果进行量化衡量。
