...
### [强化学习驱动的 LLM 选择性证据采纳——抗污染 RAG 训练（2026）](https://arxiv.org/abs/2607.20090)

**发表**：2026 年 7 月 22 日 | [arXiv:2607.20090](https://arxiv.org/abs/2607.20090)

**核心贡献**：真实 RAG 部署中，检索结果常混杂有效证据与误导性陈述甚至指令注入内容——全拒绝会丢弃有效证据，全采纳则输出错误或不安全答案。本文提出 **SelectBench** 选择性证据采纳基准与训练集，用 DAPO 强化学习（支持规则奖励和冻结语义判断器两种奖励信号）直接后训练 Qwen3.5-4B。在纠正后的 325 样本 SelectBench-v2 上，严格成功率从基础检查点的 22.46% 分别提升至 DAPO-Rule 的 25.54% 和 DAPO-DeepSeek 的 26.46%；两种策略均减少了对禁止内容的采纳，同时 MMLU 和干净 HotpotQA 上无实质性退化，证明后训练保留了通用能力。研究还识别了"提示注入跟随"作为仍需解决的关键挑战。

**与本章关系**：直接对应本章「检索增强安全性」与「Agentic RAG」知识点，SelectBench + DAPO 框架首次将 RAG 可靠性问题转化为可量化的 RL 训练任务，是继 AgentKGV（GRPO 优化检索策略）之后将强化学习用于 RAG 鲁棒性训练的最新实证，揭示了"抗污染选择能力"与"提示注入抵御"在当前 RL 训练框架下的收益差异。

---

### [规模决定 RAG 范式胜负：四类检索范式的大规模对比研究（2026）](https://arxiv.org/abs/2607.26497)

**发表**：2026 年 7 月 29 日 | [arXiv:2607.26497](https://arxiv.org/abs/2607.26497)

**核心贡献**：RAG 方法从词汇检索到图索引到 Agentic 搜索各不相同，但通常在不同基准单一规模下评估，无法比较精度-成本随语料规模的变化。本文在 28 个严格嵌套的语料层（约 1,000 到 512,000 文档）上控制变量，统一评估 BM25、稠密检索、图 RAG 和 Agentic RAG 四种范式。**核心发现**：BM25 在整个规模范围内定义了低成本 Pareto 前沿，中等规模起精度领先；纯文件系统 Agentic RAG 在最小规模略胜 BM25 但全规模落后近 20 点、查询 token 多 39 倍；用 BM25 替换 Agent 内部检索后，Agent+BM25 全规模精度达 69.4（原生 Agent 36.9，BM25 54.8）；图 RAG 在最重构建变体下每索引 token 消耗 24.6 生成 token，却在前 2% 语料就建不下去。

**与本章关系**：直接对应本章「RAG 方法对比」与「Agentic RAG」知识点，这是迄今为止覆盖规模最广、控制最严格的 RAG 范式规模化对比研究，核心结论（Agentic RAG 的内部检索瓶颈、BM25 的规模抗性）对 RAG 系统架构选型具有直接工程指导价值。

---

### [CACD：基于交叉注意力校准去重的RAG 系统优化（2026）](https://arxiv.org/abs/2607.24332)

**发表**：2026 年 7 月 27 日 | [arXiv:2607.24332](https://arxiv.org/abs/2607.24332)

**核心贡献**：RAG 系统中常见的分块策略频繁产生冗余块，使向量数据库膨胀并拖慢检索。传统余弦相似度去重将每个块压缩为单一向量，丢失了区分真重复与主题相近块所需的细粒度 token 级信息。CACD（Cross-Attention Calibrated Deduplication）对每个新块用**交叉编码器**而非池化向量检查与已保留块池的相似度，保持全程token 级精度；引入**新信息分（NIS）**——从交叉编码器注意力熵计算新块中未被已有块解释的内容比例——并跨多个候选进行多数投票。在SQuAD 1.1 完整验证集上对比 5 种现有过滤方法、9 种分块策略、18 种配置：CACD 平均去除 **9.75%** 的冗余块（与语义方法相当，远高于精确匹配），处理速度比最强基线NERExact 快 **27%**，比余弦相似度过滤快 **7倍**。

**与本章关系**：直接对应本章「RAG 系统工程优化」与「向量数据库管理」知识点，CACD 用交叉编码器 attention熵解决了 RAG 索引质量的"去重盲区"，与已收录的LatentRAG（隐空间检索）、SAG（SQL超边RAG）聚焦于检索策略不同，本文专注于索引构建阶段的数据质量，是RAG 工程全链路优化的重要补充。

---

### [推理失败之前：Agentic RAG 中的前证据程序性失败分类（2026）](https://arxiv.org/abs/2608.02011)

**发表**：2026 年 8 月 4 日 | [arXiv:2608.02011](https://arxiv.org/abs/2608.02011)

**核心贡献**：Agentic RAG 失败分析通常聚焦于"检索到错误证据后产生错误推理"，但本文指出大量失败发生在证据检索之前，属于**程序性失败（Procedural Failures）**——Agent 在尚未收集充分证据的情况下就开始推理或生成答案。本文构建了 Agentic RAG 失败分类体系，将失败模式分为：(1) 过早结束（证据不足即终止检索）、(2) 循环死锁（检索循环未终止而非答案不足）、(3) 边界模糊（工具调用序列与推理步骤之间的过渡逻辑缺失）。论文通过实测量化了各类程序性失败的占比，并提出简单运行时门控（要求至少调用 K轮工具后才允许最终答案生成）即可消除 60% 以上此类失败，且不依赖更强的基础模型。

**与本章关系**：直接对应本章「Agentic RAG」与「RAG 失败分析」知识点，本文将RAG 失败分析从"检索质量"前推到"检索行为的程序控制"层面，揭示了Agentic RAG 特有的执行逻辑缺陷——与已收录的CACD（索引去重优化）、AgentKGV（检索-RL 联合优化）聚焦不同，本文专注于检索行为流程本身的可靠性，对构建鲁棒 Agentic RAG 系统具有直接工程价值。

---
### [意图引导解码：鲁棒检索增强生成的信源信任仲裁（2026）](https://arxiv.org/abs/2608.16515)

**发表**：2026 年 8 月 17 日 | [arXiv:2608.16515](https://arxiv.org/abs/2608.16515)

**核心贡献**：RAG 通过将生成接地于外部证据来改善 LLM，但同时引入"信源信任"难题——检索内容可能有用、无关乃至误导性。现有 RAG 系统对检索证据采用固定信任策略，要么过度依赖错误上下文，要么在用户明确需要上下文跟随时反而不用上下文。IGD（意图引导解码）提出根据用户意图**仲裁**检索上下文与参数记忆的框架：通过答案级过滤和 token 级校正将最终解码轨迹导向检索上下文或参数记忆之间。在三个真实性 QA 基准和三个事实冲突基准（五个 LLM）上评估，IGD 在事实冲突基准上相对直接 RAG 的事实恢复率提升最高 **65.4个百分点**，同时保持甚至改善了严格上下文跟随行为。

**与本章关系**：直接对应本章「RAG 可靠性」与「检索上下文冲突处理」知识点，IGD 揭示了 RAG 中"何时信任检索内容、何时信任参数知识"的核心张力，从解码策略层面提供了动态仲裁机制，与已收录的"前证据程序性失败"（控制检索行为流程）互补——前者保障检索流程完整，本文保障生成阶段的真实性。

---
### [LivingRAG：为 Graph RAG 添加可写可复用推理经验的框架（2026）](https://arxiv.org/abs/2608.25960)

**发表**：2026 年 8 月 26 日 | [arXiv:2608.25960](https://arxiv.org/abs/2608.25960)

**核心贡献**：现有 RAG 系统对每个查询独立处理，推理后丢弃有用的 LLM 推断结果——后续相关查询需要从头检索和推理。LivingRAG 在基于图的检索主干上增加**可写经验库（writable experience store）**：经验以两种方式复用——存储的图信号帮助检索找到此前有用的实体和段落，存储的摘要为答案生成提供参考推理模式。通过分析在线 QA 流，作者发现共享实体、图邻域和问题模板三类信号均产生可复用的推理经验。在多跳 QA 基准上，LivingRAG 相比强 RAG 基线提升精度，并在复用先验经验时减少生成 token 消耗。

**与本章关系**：直接对应本章「GraphRAG」与「知识图谱检索」知识点，LivingRAG 的核心洞见是"推理过程本身是有价值的资产"——将 GraphRAG 从无状态查询系统升级为有记忆的持续学习系统，与已收录的 IGD（检索结果动态仲裁）形成互补，共同指向 RAG 系统的长期可靠性。

---

### [CEMMKG：多粒度上下文增强的多模态知识图谱 RAG 框架（2026）](https://arxiv.org/abs/2608.25986)

**发表**：2026 年 8 月 26 日 | [arXiv:2608.25986](https://arxiv.org/abs/2608.25986)

**核心贡献**：多模态知识图谱（MMKG）为 GraphRAG 引入了跨模态知识，但现有 MMKG-RAG 方法在提取视觉信息时对文本上下文利用不足，导致图像与文本之间的语义鸿沟限制了检索质量。CEMMKG 提出**上下文增强 MMKG** 构建框架：为每张图像附加本地上下文（不只是周围文字，还包括语义相关句子）和全局上下文（整段摘要），并引入多粒度本地上下文设计以捕获不同细节层次的语义信息。在视觉中心数据集上验证了 CEMMKG 在多种 MMKG-RAG 方法上的普遍提升效果。

**与本章关系**：直接对应本章「多模态 RAG」与「知识图谱检索增强」知识点，CEMMKG 揭示了多模态 RAG 中"视觉信息提取缺乏文本上下文"这一系统性短板，其多粒度上下文增强方案对构建生产级多模态知识库具有直接参考价值。

---

### [CHyD：将原文约束注入解码过程，消除 RAG 引用幻觉（2026）](https://arxiv.org/abs/2609.10046)

**发表**：2026 年 9 月 13 日 | [arXiv:2609.10046](https://arxiv.org/abs/2609.10046)

**核心贡献**：RAG 系统中的引用幻觉问题（即生成内容"引用"了实际上并不包含该信息的文档段落）难以从训练层面根治。CHyD（Constrained Hybrid Decoding）将原文引用约束直接写进解码阶段：在 token 生成时施加约束，确保声称来自原文的摘录必须是源文档中的精确字面子序列；同时用第二重验证确认该摘录是否真正回答了问题。两项约束均通过后，引用框才对用户有效。在多个 QA 基准上，CHyD 大幅降低引用幻觉率，且与底层 LLM 无关，可作为即插即用的解码层叠加在任意 RAG 系统上。

**与本章关系**：直接对应本章「RAG 可靠性」与「引用准确性」知识点，CHyD 将"引用幻觉"从模型生成问题转化为解码约束问题——"引用框中的内容是否来自原文"可以被机械检验，而非依赖模型的诚实性；与已收录的 IGD（信源信任仲裁）形成互补，IGD 决定何时信任检索内容，CHyD 保证信任的内容确实来自原文，两者共同构成 RAG 真实性保障的完整链路。

---

### [DCI：超越语义相似度——用直接语料交互重构 Agentic 检索（2026）](https://arxiv.org/abs/2605.05242)

**发表**：2026 年 5 月（NeurIPS 2026 Spotlight） | [arXiv:2605.05242](https://arxiv.org/abs/2605.05242)

**核心贡献**：传统 RAG 将语料访问压缩为单次 top-k 相似度检索，对 Agentic 搜索构成根本瓶颈——精确词法约束、稀疏线索组合、局部上下文验证和多步假设修正均难以实现。DCI（Direct Corpus Interaction，德州农工/斯坦福/UW 等多校，NeurIPS 2026 Spotlight）让 Agent 直接用通用终端工具（grep、find、file read、轻量脚本）与原始语料交互，完全绕开嵌入模型与向量索引。在 BrowseComp-Plus（深度研究）、多跳 QA 和 BRIGHT/BEIR IR 排名等多个基准上，DCI 以更低 API 成本大幅超越强 sparse/dense/reranking 基线，且无需预建索引，天然适应持续演化的本地语料库。

**与本章关系**：直接对应本章「Agentic RAG」与「检索接口设计」知识点，DCI 将检索研究的问题从"如何选更好的检索器"重新定义为"如何设计更高分辨率的检索接口"——当 Agent 推理能力足够强时，直接操作原始语料比依赖压缩后的向量表示更精确、更灵活，与已收录的 CHyD（解码层引用约束）和 IGD（信源信任仲裁）形成从检索接口到生成质量的完整可靠性链路。
