...
```
4.1 为什么需要工具？        → 先建立直觉：LLM 的三个"先天残疾"
4.2 Function Calling 机制   → 核心章节：5 步流程 + 最小可运行代码
4.3 自定义工具的设计原则    → 不是怎么写代码，而是怎么思考设计
4.4 工具描述的编写技巧      → description 写得好不好，Agent 智商差一倍
4.5 实战：搜索引擎+计算器   → 把前面学的全部串起来
4.6 论文解读               → 学术前沿：Toolformer / Gorilla / ToolLLM
```
