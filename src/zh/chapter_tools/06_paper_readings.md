...
- 开源模型（如 Qwen、Llama）通过优质工具调用数据微调后，能力可以接近 GPT-4.1
...
> 💡 **前沿趋势（2025-2026）**：工具调用领域的三大趋势：① **MCP 生态爆发**：Anthropic 主导的 MCP 协议已被 OpenAI、Google、微软等巨头采纳，成为工具接入的行业标准（详见第 15 章）；② **动态工具发现**：从"预定义工具列表"向 RAG-MCP 式的"按需检索、按需加载"演进；③ **开源模型追赶**：通过 ToolACE 等数据合成方法，Qwen 3、Llama 4 等开源模型的工具调用能力已接近 GPT-4.1。
...
### [Governed MCP：基于 Logit 安全原语的内核级工具治理](https://arxiv.org/abs/2604.16870)

**发表**：2026 年 4 月 18 日 | [arXiv:2604.16870](https://arxiv.org/abs/2604.16870)

**核心贡献**：将 AI Agent 的工具调用类比为操作系统的"系统调用"，提出在操作系统内核层面实现 MCP 工具治理网关 Governed MCP。核心设计是六层流水线（Schema 验证 → 信任等级 → 速率限制 → 对抗过滤 → ProbeLogits 语义门控 → 宪法策略匹配），附带 Blake3 哈希审计链；其中 ProbeLogits 语义门控是核心承重层——移除后 F1 从 0.773 暴跌至 0.327（ΔF1 = -0.446）。系统在用 Rust 编写的裸金属 x86_64 操作系统 Anima OS 中实现，从结构上杜绝了用户空间的 10 行绕过脚本攻击。

**与本章关系**：对应本章 4.4 节「工具安全与权限控制」，从操作系统内核视角提供了比应用层防护更强的工具调用安全保障方案，也是 MCP 协议安全治理方向的重要进展。
