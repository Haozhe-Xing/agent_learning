...
### [SING：大规模工具生态中意图感知的主动工具发现](https://arxiv.org/abs/2606.16591)

**发表**：2026 年 6 月 15 日 | [arXiv:2606.16591](https://arxiv.org/abs/2606.16591)

**核心贡献**：当工具生态扩展到数千个 API 时，穷举注入所有工具 Schema 代价巨大，且假设 Agent 在"封闭世界"中操作。SING 提出**意图-工具图**（Intention-Tool Graph）：将用户意图、工具能力和工具协作模式三者关联，在任务执行过程中根据不断演化的状态（子目标分解、工具观测、新诱导目标）动态检索所需工具，而非一次性固定工具集。在 7,471 个工具的统一语料上，SING 提升 Global Recall@5 达 59.8%，下游成功率最高提升 28.9%，同时减少完整工具 Schema 曝露达 99.8%。

**与本章关系**：直接对应本章「工具的检索与选择」和「大规模工具生态管理」知识点，是 RAG 式工具检索在长视野 Agentic 任务中的最新突破，从"静态工具库"走向"动态工具发现"。

---

### [LedgerAgent：结构化状态跟踪与策略合规工具调用 Agent](https://arxiv.org/abs/2606.20529)

**发表**：2026 年 6 月 18 日 | [arXiv:2606.20529](https://arxiv.org/abs/2606.20529)

**核心贡献**：客服域工具调用 Agent 需同时维护跨轮次任务状态并遵守领域策略。现有 Agent 不单独表示任务状态——观测、工具返回和策略指令混入 prompt，每次决策时从中重建状态，导致两类典型失败：检索到了正确事实但仍基于过期/缺失信息做决策，以及语法合法的工具调用违反策略。LedgerAgent 在推理时引入独立的**账本（Ledger）**维护已观测任务状态，并在执行环境变更类工具调用前用账本验证状态相关策略约束，主动拦截策略违反。在四个客服域、多个开放/封闭权重模型上，LedgerAgent 在多轮一致性严格指标下获得最大提升。

**与本章关系**：对应本章「工具调用可靠性」与「工具调用中的策略合规」知识点，是将显式状态管理引入工具调用循环的最新工程化探索，直接解决了多轮工具执行中"上下文漂移"与"隐式状态错乱"的核心问题。
