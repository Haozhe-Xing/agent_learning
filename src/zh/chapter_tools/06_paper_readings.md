...
### [SpatialCLI：先用工具推理，再内化工具能力（2026）](https://arxiv.org/abs/2607.27703)

**发表**：2026 年 7 月 30 日 | [arXiv:2607.27703](https://arxiv.org/abs/2607.27703)

**核心贡献**：通用视觉语言模型（VLM）能理解整体任务但常遗漏决定成败的视觉细节，而专用视觉模型能捕获细节却无法转化为任务决策——两类能力存在根本性错配。SpatialCLI 提出**三阶段工具使用→能力内化**框架：(1) **Call阶段**：将专用空间视觉模型暴露为工具供VLM 调用以增强感知；(2) **Learn 阶段**：通过冷启动 SFT + Agentic RL 提升工具调用能力；(3) **Internalize 阶段**：将成功的工具调用轨迹语言化，蒸馏为模型无需工具的内生感知能力。在 MindCube 上，SpatialCLI 将Qwen3-VL-8B 从 29.3% 提升至使用工具时的 **84.6%**（超过 GPT-5.6 Sol+工具的 72.1%），内化后无工具保留 **73.8%**。

**与本章关系**：直接对应本章「工具调用与能力学习」知识点，SpatialCLI 展示了"工具不只是运行时调用对象，更是训练时的能力教师"的全新视角——将工具使用成功轨迹蒸馏为参数化内生能力，与已收录的 OpenAgent（工具泛化脆弱性分析）互补：前者诊断静态训练的上限，本文提供了突破这一上限的新路线。

---

### [ReCache：工具增强 LLM Agent 的 KV 缓存复用与压缩（2026）](https://arxiv.org/abs/2608.19662)

**发表**：2026 年 8 月 20 日 | [arXiv:2608.19662](https://arxiv.org/abs/2608.19662)

**核心贡献**：工具增强 Agent 每次调用工具都需要重新编码工具 schema 和资源描述，造成大量重复的 KV 缓存计算和内存占用。ReCache 提出**资源感知注意力（resource-wise attention）**——将工具资源的 KV 块从跨资源交互中解耦，生成组合不变（composition-invariant）的 KV 块，从而可跨调用直接复用和压缩。在 7 个公开工具使用数据集构建的基准上，ReCache 在匹配稠密调用性能（Inv-F1 82.3% vs. 82.4%）的同时，将 KV 张量内存分配降低 **92.43%**，注意力计算加速 **1.423×**；资源感知注意力组件单独可实现首 token 时间 **3.655×** 加速。

**与本章关系**：直接对应本章「工具调用效率优化」与「工具 schema 编码」知识点，ReCache 从系统层面解决了频繁工具调用的 KV 缓存冗余问题，揭示了工具资源描述的"组合不变性"这一关键工程属性，是继 SpatialCLI（工具→能力内化）之后工具调用效率方向的最新成果。

---
### [JIT-Agent：按需合成任务自适应 Agent Harness 的智能体（2026）](https://arxiv.org/abs/2608.25593)

**发表**：2026 年 8 月 26 日 | [arXiv:2608.25593](https://arxiv.org/abs/2608.25593)

**核心贡献**：Agent 的实际能力不只取决于底层模型，还强烈依赖 Harness——包括记忆管理、规划策略、动作协议和工具/技能编排，而 Harness 设计目前仍是手工的、任务特定的、根本无法规模化的。JIT-Agent 提出一个专门训练的 Harness 智能体：把 Harness 形式化为可机器生成的四模块协议组合体，训练 JIT-Agent 按需合成任务自适应 Harness、修复执行错误，并通过从历史 Harness 配置档案蒸馏性能信号来自我演化。使用 JIT-Agent 后，DeepSeek-V4-Flash 在 DeepSearchQA 上超越 GPT-5.6（+9.1），GLM-5.2 提升高达 +20.2 分；生成的 Harness 与 OpenCode、Claude Code 等成熟运行时性能相当。

**与本章关系**：直接对应本章「工具/技能编排」与「Agent Harness 设计」知识点，JIT-Agent 将 Harness 本身变成一个可训练、可迁移的组件——意味着工具编排不再是硬编码逻辑，而是可通过学习自动适配任务，开创了"Harness 智能"这一与模型 scaling 正交的新维度。

---

### [Agent Seer：从 MCP 工具规格自动合成 Agent 评测场景（2026）](https://arxiv.org/abs/2608.26133)

**发表**：2026 年 8 月 27 日 | [arXiv:2608.26133](https://arxiv.org/abs/2608.26133)

**核心贡献**：评估使用外部工具的 AI Agent 需要真实的测试场景，但手工构建场景需要深度领域知识、无法跨工具生态扩展，且静态基准无法跟踪 API 演化。Agent Seer 发现工具规格（函数名、自然语言描述、带类型的参数 schema）已蕴含足够语义信息，无需手工策划即可合成真实评测场景。从单个 MCP 规格出发，无示例、无 live 工具访问、无领域微调，自动丰富 schema、生成带合成工具输出的分级场景，并扩展为多轮对话。在 7 个跨领域 MCP 规格上评估，参数 schema 复杂度是质量变化的最强相关因素，参数值精度是主要失败模式。

**与本章关系**：直接对应本章「工具调用评估」与「MCP 协议」知识点，Agent Seer 解决了工具型 Agent 评测中的"冷启动"问题——新 API 上线就能自动生成评测数据集，且场景质量随工具规格本身演化，为构建可持续、低成本的工具调用评估基础设施提供了直接可用的方案。

---

### [推测性宏提交（SMC）：两层 Agent 架构下多步预执行加速工具调用（2026）](https://arxiv.org/abs/2609.03236)

**发表**：2026 年 9 月 3 日 | [arXiv:2609.03236](https://arxiv.org/abs/2609.03236)

**核心贡献**：工具使用型 Agent 的延迟不仅来自模型推理，更来自"调用工具→等待结果→再决策"的串行循环。SMC（Speculative Macro Commit）采用两层架构：大型"权威 Actor 模型"负责官方轨迹，轻量"推测起草者模型"在隔离环境快照中**持续预测并预执行**未来多步动作链；从训练轨迹挖掘的多步宏模式与起草者预测匹配后，一旦权威模型的下一步与起草者预测吻合，SMC 立即将后续已预执行步骤（含真实观测结果）提交进官方轨迹。在 τ²-Bench 上相比串行执行降低延迟 **18.59%**，AppWorld 上降低 **44.9%**，任务准确率基本持平。

**与本章关系**：直接对应本章「工具调用效率优化」与「多步工具链执行」知识点，SMC 将推测执行从"单 token 预测"扩展到"多步工具动作链"级别，首次量化了宏级别预执行对真实工具型 Agent 延迟的改善效果；与已收录的 ReCache（KV 缓存复用）共同构成工具调用系统性能优化的方法图谱——ReCache 减少 schema 编码开销，SMC 减少串行观测等待时间。

---

### [Agent 过度信任工具：跨 14 个模型的工具可靠性测量（2026）](https://arxiv.org/abs/2609.05587)

**发表**：2026 年 9 月 5 日 | [arXiv:2609.05587](https://arxiv.org/abs/2609.05587)

**核心贡献**：Yang、Song 等人（韩国科学技术院等）系统测量了 LLM Agent 对不可靠工具输出的依赖程度：故意污染 14 个主流模型在 5 类工具上的返回结果，测量 Agent 采纳被污染内容的比例。**核心发现**：所有被测模型对每类工具的平均采纳率均超过 1/3；网页搜索工具的采纳率高达 **68%**；最反直觉的发现是——模型内部推理链已正确恢复了答案，但最终输出仍然重复了被污染的工具结果，说明问题不是能力不足，而是工具输出与最终答案之间缺少验证层（即使模型"知道"工具结果有误，也不可靠地将正确判断传递到最终答案）。

**与本章关系**：直接对应本章「工具调用可靠性」与「工具输出验证」知识点，本文将"工具返回值 ≠ 可信证据"的直觉量化为严格实验结果：68% 的网页搜索污染采纳率意味着在没有独立验证层的情况下，Agent 从可疑来源拉取信息时超过一半的概率会把错误输入直接变成输出；与已收录的 PlanFence（计划依赖验证）形成互补——前者守住"依据是否仍然有效"，本文则指出"工具返回值本身是否可信"需要被当作独立问题处理。

---

### [ToolLoop：分解生成与动态自反馈的闭环工具调用数据合成（2026）](https://arxiv.org/abs/2609.09072)

**发表**：2026 年 9 月 8 日 | EMNLP 2026 Main Conference | [arXiv:2609.09072](https://arxiv.org/abs/2609.09072)

**核心贡献**：现有工具调用数据合成方法普遍采用"先生成、再筛选"流程，缺少对中间过程的分阶段验证。ToolLoop 将合成过程拆分为三个阶段：①目标函数组合采样（先确定预期调用哪些函数）；②用户问题反向生成（由目标函数语义推出用户意图）；③工具调用正向生成（生成参数对齐的具体调用），每阶段均引入动态自反馈——验证失败时围绕当前阶段修正而非丢弃整条样本。函数采用语义向量 K-means 聚类 + 模型引导采样，确保多工具候选集具有语义关联。相比整体筛选方式，ToolLoop 在单次调用和多次调用场景下均显著提升工具选择准确率和参数正确率。

**与本章关系**：直接对应本章「工具调用训练数据构建」与「多步工具调用」知识点，ToolLoop 解决了工具调用数据合成的关键问题——"生成→筛选"无法定位错误来源，而分阶段约束 + 闭环修正能在不增加采样成本的前提下大幅提升数据质量；对需要从头构建工具调用训练集的团队，ToolLoop 提供了一套直接可复用的数据工程流水线。

---

### [工具幻觉闭世界解析：LLM Agent 调用不存在工具的分类法与检测基准（2026）](https://arxiv.org/abs/2609.19425)

**发表**：2026 年 9 月 19 日 | [arXiv:2609.19425](https://arxiv.org/abs/2609.19425)

**核心贡献**：工具增强型 Agent 存在一类无现有防御机制覆盖的失败模式——**调用根本不存在的工具**，或传入任何 Schema 都未声明的参数。现有防御要么负责选择正确工具（Selection），要么约束 Agent 对真实工具的使用（Gating），两者都以"调用的工具真实存在"为前提，而幻觉调用从定义上就不在任何门控决策中——门控根本无法拒绝它。论文提出**工具幻觉五类分类法（H1-H5）**，以及作为参照系的"解析台阶"（Resolution Rung）：一个无需训练的闭世界解析器（注册表成员检查 + 签名核验），必须放在所有因果门控**之前**。跨 10 个模型的实测发现 **322 个真实幻觉**；在 MCP 多服务器合并场景下额外发现 154 个幻觉，来源是命名空间合并产生的碰撞与遮蔽（M1-M5 第二分类法）。论文同时发布可安装的 **HTB（Hallucinated-Tools Benchmark）**，任意解析器均可提交评测。

**与本章关系**：直接对应本章「工具调用可靠性」与「工具调用安全性」知识点，本文揭示了工具调用防御体系中一个结构性盲区——"幻觉调用"不是选错工具，而是调用了注册表中不存在的实体，必须用闭世界成员检查而非 Schema 验证来防御；与已收录的「Agent 过度信任工具」（2609.05587，污染内容采纳率 68%）形成互补，共同指向工具调用的两类安全失效：幻觉调用（本文）和污染输出采纳（2609.05587）。

---

### [Stale 不等于 Unsafe：基础设施状态竞争下工具调用守卫的精度分析（2026）](https://arxiv.org/abs/2609.29522)

**发表**：2026 年 9 月 25 日 | [arXiv:2609.29522](https://arxiv.org/abs/2609.29522)

**核心贡献**：工具调用 Agent 会修改调度器、数据管道、对象存储和访问控制系统。Agent 读操作到提交之间，外部状态可能改变——但并非每次改变都使提交不安全。论文将竞态分为**使安全谓词失效的竞态**（invalidating races）与**无关竞态**（predicate-preserving/irrelevant races），并测量三种守卫粒度（全局 epoch、读集版本、语义提交谓词）对二者的区分精度。在 16 个基础设施任务上的 3456 条轨迹实验发现：所有守卫均能消除不安全提交；但**基于新鲜度的守卫误封 92–95% 的良性竞态**，导致多达 43% 的安全任务被放弃；而完整语义谓词守卫无误封。模型侧信号无法替代语义合约：口头置信度错误校准（ECE ≈ 0.37），谨慎性提示对不安全率几乎无影响。

**与本章关系**：直接对应本章「工具调用安全性」与「工具副作用控制」知识点，本文量化了工具守卫设计的核心权衡——过于保守的守卫会大幅降低 Agent 的有效工具执行率，而精度不足的守卫又存在安全风险；"语义合约优于新鲜度启发式"的结论为工具调用框架设计提供了直接可操作的架构原则。
