...
### [EnvHarness：让静态训练环境随 Agent 一起进化（2026）](https://arxiv.org/abs/2608.19880)

**发表**：2026 年 8 月 20 日 | [arXiv:2608.19880](https://arxiv.org/abs/2608.19880)

**核心贡献**：现有 Agent 训练环境由人工编写，任务和难度在部署后冻结——无论 Agent 学会了什么，环境提供的任务总相同，导致模型迅速刷完固定环境并停止进步。EnvHarness（来自 Google + 圣路易斯华盛顿大学）在静态环境外加一层**可编程组件**（Harness），通过标准接口调整初始状态、交互规则和任务长度，同时保留原有人工验证器；配套的 EnvRigger 将目标策略当黑盒，从成功/失败轨迹中诊断能力缺陷，自动写入和验证新的 Harness 组件。跨 4 个领域 5 个基准，EnvHarness 在留出任务上最高提升 **+9.0 点**，平均执行步数减少 **9.8%**，并为强化学习提供更优的优化信号，实现策略与环境的持续协同进化。

**与本章关系**：直接对应本章「长时程规划能力培养」与「Agent 训练范式」知识点，EnvHarness 将训练环境从"固定资产"变成"随 Agent 成长而自适应演化的动态系统"，是解决 RL 后训练"环境饱和"问题的重要工程方向，与已收录的 Living-Harness（自进化 Harness 运行框架）形成互补——后者用情节记忆演化运行时，本文用 Harness 层演化训练环境。

---

### [FACET：可执行任务合成中的意图与环境对齐框架（2026）](https://arxiv.org/abs/2608.18580)

**发表**：2026 年 8 月 19 日 | [arXiv:2608.18580](https://arxiv.org/abs/2608.18580)

**核心贡献**：合成 Agent 训练任务面临两大痼疾：多阶段生成时（先写指令、再写解法、再写验证器）会造成前后假设不一致，导致任务根本无法完成；以及 LLM 合成过程中容易丢弃源技能蕴含的目标、依赖关系和状态转移约束，生成"好看但不可执行"的任务。FACET（Fine-grained Agentic Construction of Executable Tasks，来自中科大）提出**以可执行环境为中心的倒序生成**：先将相关 Agent 技能组合为信息丰富的场景，再实现并修复真实执行容器状态——容器状态作为共同基础，统一派生指令、参考解法和验证器，确保三者逻辑一致；执行验证和定向修复只针对失败的组件，不重新生成已通过的部分。在 Terminal-Bench 2.1 基准上，跨多个规模的模型微调均获得持续提升，消融实验证实"基于环境的任务构建"和"解法-验证器对齐"是核心增益来源。

**与本章关系**：直接对应本章「Agent 训练数据构建」与「规划与执行对齐」知识点，FACET 揭示了"容器即真理来源"的数据工程范式——让 Agent 的规划目标、执行路径和验证逻辑在可运行环境中协同定义，是继 EnvHarness（环境层改造）和 SPADE（环境生成）之后解决"训练数据质量"问题的最新突破，三者共同构成 Agentic RL 训练基础设施的完整图景。

---

### [PILOT：在运行中实时自我改进的长时程 Agent 监督-工作者 Harness（2026）](https://arxiv.org/abs/2608.26530)

**发表**：2026 年 8 月 27 日 | [arXiv:2608.26530](https://arxiv.org/abs/2608.26530)

**核心贡献**：当前主流自我改进方法在任务结束后才处理经验，无法在运行中及时纠偏、也无法立即验证学到的教训。PILOT 提出**监督-工作者 Harness**，通过两个耦合机制实现实时自我改进：① **实时转向**（live steering）——独立的监督者在执行过程中随时重定向或中止活跃工作者，而非等任务结束后复盘；② **实时自进化**（live self-evolution）——将运行期间暴露的执行过程与失败模式提炼为可复用技能和记忆，写回 Harness。在 2 个冻结骨干模型、3 个基准的 6 组配置中，PILOT 五组排名第一；Terminal-Bench 2.0 上比同类 Harness 高 **9.8 个百分点**，自改进模式下 GLM-5.1 提升 **14.6 点**、Kimi-K2.6 提升 **12.4 点**，输出 token 数分别下降 **42.9%** 和 **47.4%**，每百万输出 token 成功评测数提升 **110.3%** 和 **134.0%**。

**与本章关系**：直接对应本章「Agent Harness 设计」与「长时程任务可靠性」知识点，PILOT 的核心创新是"监督-工作者分离使得中途重定向成为可能"——这是单 Agent 自纠错架构从根本上无法实现的能力边界；实时技能蒸馏将规划失败直接转化为下一步执行的经验，是 Agentic 规划层"从失败中实时学习"的最新范式。

---

### [PlanFence：依赖范围验证协议，解决分布式 Agent 团队的陈旧计划执行问题（2026）](https://arxiv.org/abs/2609.03340)

**发表**：2026 年 9 月 3 日 | [arXiv:2609.03340](https://arxiv.org/abs/2609.03340)

**核心贡献**：分布式 LLM Agent 团队可能读到最新共享事实，却仍执行基于过时假设派生的计划——某 Agent 的计划依赖需求 r3，另一 Agent 随后提交 r4 更新，执行 Agent 收到 r4 但计划仍来自 r3 时代（"陈旧计划执行"）。PlanFence 引入**依赖范围动作验证**：每个计划显式引用其决策所依据的公共记录；执行 Agent 在发起外部操作前仅验证能影响该动作的相关记录（而非全量刷新），验证不通过时触发局部重规划或阻塞。在 30 个含计划后修订的受控直播工作流中，无 PlanFence 的执行器每次都在陈旧计划上行动，PlanFence 则零陈旧动作完成全部任务，且主动同步策略可进一步降低协调开销。

**与本章关系**：直接对应本章「多 Agent 协同规划」与「Agent 计划一致性」知识点，PlanFence 区分了"状态新鲜度"和"计划有效性"这两个经常被混同的概念——读到最新事实不等于当前计划仍然有效；精确的依赖范围追踪而非全量状态同步，解决了分布式 Agent 规划中的"版本一致性"问题，与已收录的 PILOT（实时重定向）形成互补，共同指向长时程任务中计划可靠性的工程方向。

---

### [程序图：面向 LLM 智能体的自演化执行结构（2026）](https://arxiv.org/abs/2609.09153)

**发表**：2026 年 9 月 8 日 | [arXiv:2609.09153](https://arxiv.org/abs/2609.09153)

**核心贡献**：Google、北京大学等团队提出程序图（Procedural Graph）：类比知识图谱将事实组织为「实体—关系—实体」三元组回答"是什么"，程序图将程序性知识组织为「步骤—关系—步骤」三元组回答"该做什么"。每个决策步骤中，框架定位 Agent 当前所在节点，引导模型将周边子图转化为步骤级情境建议（偏置而非强制）。图是**自演化**的：LLM 精炼器对比成功与失败轨迹，编辑拓扑和属性，仅提交能通过保留集验证的修改，同时保留被拒绝的修改记录以避免重蹈覆辙。跨多个数据集、任务类型和模型，程序图持续优于基于记忆的基线，自演化循环在无需人工工程的情况下进一步提升性能。

**与本章关系**：直接对应本章「Agent 规划结构」与「程序性知识管理」知识点，程序图将规划从"无约束生成 + 不断累积历史"升级为"有结构的执行先验 + 持续自演化"——是解决长轨迹中 Agent 丢失目标、工具调用乱序、重复无效动作等规划退化问题的最新架构方案。

---

### [Auto-RecSys：Meta 将自主研究 Agent 部署于工业级推荐系统的生产实践（2026）](https://arxiv.org/abs/2609.10922)

**发表**：2026 年 9 月 10 日 | [arXiv:2609.10922](https://arxiv.org/abs/2609.10922)

**核心贡献**：Meta 提出 Auto-RecSys——针对工业级推荐模型（单次训练耗时可达数天）的长时程自主研究系统，正面解决两大挑战：① **超长反馈循环**（串行迭代极慢）：通过分布式异步执行实现多服务器并行实验；② **系统复杂性**（大规模配置+脆弱基础设施依赖+多天 GPU 任务）：通过跨服务器中央记忆保证崩溃恢复，以及认知-程序分离设计——自然语言技能文件驱动 LLM 推理，确定性脚本保证操作正确性。双循环自进化架构：执行演化环把失败尝试和成功管线写入模型专属 Playbook，每轮迭代所需主要修复次数从 **4.0 降至 1.3**；想法演化环让实验结果反哺下一轮假设生成。

**与本章关系**：直接对应本章「长时程规划」与「Agent Harness 设计」知识点，Auto-RecSys 是将 Agent 规划能力部署于高代价真实生产环境的最完整案例之一——其认知-程序分离设计（推理用自然语言技能文件、操作用确定性脚本）和双循环自进化架构，与本章讨论的"规划可靠性"和"从失败中学习"主题直接对应，是验证 Long-Horizon Agent 在工业规模任务上实际有效的关键实证参考。

---

### [AEWM：不要预测工具输出，去编辑 Agent 的错误信念（2026）](https://arxiv.org/abs/2609.28416)

**发表**：2026 年 9 月 25 日 | [arXiv:2609.28416](https://arxiv.org/abs/2609.28416)

**核心贡献**：现有 LLM Agent 世界模型专注于预测工具调用的返回值，而 AEWM（Agent-Editing World Model，RUCAIBox）指出这并非长时程 Agent 失败的根本原因——真正有害的是累积在上下文中的"陈旧计划与无根据假设"（任务状态污染）。AEWM 提出 **Action Judge**（将历史动作分为 Critical / Exploratory / Noisy 三类）和 **State Revision**（对 Noisy 动作重写其推理与行动，而非仅批评）两个模块，并以 EditAct 接入实际执行，直接修改下一步决策的上下文起点而非仅在末尾追加批注。在搜索、终端、软件工程等 6 个基准和 3 个 Agent 骨干上，EditAct 相比最强基线提升 **3.2~6.7 点**；基于验证 EditAct 轨迹的拒绝采样微调超越普通 self-RFT **2.2~2.6 点**，且推理时无需在线 AEWM 即可保持增益。

**与本章关系**：直接对应本章「Agent 规划的失败恢复」与「上下文污染」知识点，AEWM 将自我纠错的核心从"在上下文末尾追加批评"升级为"删除并重写错误推理"——旧的错误信念留在上下文中会持续干扰后续决策，State Revision 从根本上消除这一信号源，与已收录的实时故障检测（在线触发修复）形成互补，共同指向规划层鲁棒性的不同纵深。

---

### [Qwen-Planner-Agent：让 AI 自主构建下一代移动规划 Agent（2026）](https://arxiv.org/abs/2609.29892)

**发表**：2026 年 9 月 25 日 | [arXiv:2609.29892](https://arxiv.org/abs/2609.29892)
