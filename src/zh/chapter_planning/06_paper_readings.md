...
> 🗓️ 本节由每日自动更新任务维护，最近更新：**2026 年 8 月 5 日**
...
### [Living-Harness：将每条执行轨迹转化为持久工作流知识的自进化 Agent Harness（2026）](https://arxiv.org/abs/2607.26598)

**发表**：2026 年 7 月 29 日 | [arXiv:2607.26598](https://arxiv.org/abs/2607.26598)

**核心贡献**：Agent 运行后即便在当次情节内恢复了失败，同一执行失败仍会在后续任务中反复出现——因为情节后反馈几乎从不修订指导未来交互的持久 Harness（工具、上下文、记忆、工作流结构）。Living-Harness 提出**自进化 Agent Harness**：由 **Evolution-SOP**（标准操作程序）引导，从每条完整轨迹及其评估信号中提取情节抽象和结构化更新证据，写入两类互补程序性知识——**情节记忆**（记录触发条件、失败模式与恢复动作）和**状态图**（记录状态节点、修复边与转移规则）。工具和基础上下文保持冻结，仅程序性修复跨演化周期累积；更新后的 Harness 状态被检索用于指导后续交互，且支持跨骨干模型复用已演化的 Harness 状态。在 τ²-Bench 和 MultiWOZ-2.4 派生的 8 个交互环境上，Living-Harness 相比最强交互基线分别提升 Pass@1 **+10.07pp** 和 **+9.91pp**。

**与本章关系**：直接对应本章「任务规划与自我改进」知识点，Living-Harness 将静态执行框架升级为从失败经验自动进化的规划知识库——状态图和情节记忆充当了可累积的工作流规划经验，是继 GATS（世界模型规划）和 ATG（原子任务图）之后规划框架从"一次性提示"向"经验驱动演化"方向迈进的最新实证。

---

### [短思快推、智能延迟、行动、循环：边缘 LLM Agent 的校准推理与不确定性感知决策（2026）](https://arxiv.org/abs/2607.26865)

**发表**：2026 年 7 月 30 日 | [arXiv:2607.26865](https://arxiv.org/abs/2607.26865)

**核心贡献**：在边缘端部署的 LLM Agent 面临两个相互约束的优化目标：推理 token 预算（越多推理越好但越贵）和云端 API 延迟预算（延迟到云端处理更准确但有成本/延迟惩罚）。本文提出联合优化框架：(1) **校准推理截断**——不再固定思维链长度，而是根据任务不确定性动态截断，"思维够用即止"；(2) **不确定性感知延迟决策**——当本地推理置信度低于阈值时，将任务延迟到云端更强模型处理，并显式建模延迟决策的期望收益减去成本。两者在显式奖励约束下联合优化，在 AgentBench 和 τ-Bench 边缘设置上，以约一半推理 token 成本匹配或超越固定链长基线，延迟决策准确率相比启发式规则提升 18 个百分点。

**与本章关系**：直接对应本章「推理与规划的效率优化」与「任务路由决策」知识点，本文首次将"何时延迟到更强模型"建模为不确定性驱动的显式优化问题，是 Plan-and-Execute 框架在资源受限场景下的最新延伸，对构建低成本、高可靠 Edge-Cloud 混合 Agent 系统具有直接架构参考价值。

---

### [LLM Agent 故障的实时检测与修复（2026）](https://arxiv.org/abs/2608.02464)

**发表**：2026 年 8 月 4 日 | [arXiv:2608.02464](https://arxiv.org/abs/2608.02464)

**核心贡献**：Agent 任务失败后的常见处理方式是重新采样或逐步调用 LLM 裁判员评判，前者修复率低（仅约 16%），后者开销高昂。本文提出基于廉价单类遥测监控器（无需逐步 LLM 评判）的**实时故障检测与修复闭环**：监控器持续观测 Agent 执行轨迹的系统级信号（动作类型分布、工具调用频率、外部依赖响应码等），一旦检测到失败特征即触发有针对性的回滚重跑（针对失败原因选择最优恢复点，而非简单重置）。在基准评估中，目标回滚将可修复失败率从重采样的 16% 提升至 **45%**，整体任务成功率从 52% 提升至 **73%**，且监控器推理成本远低于 LLM 裁判员方案。

**与本章关系**：直接对应本章「Agent 规划的失败恢复」与「任务执行可靠性」知识点，本文提供了一套将"检测-触发-修复"三个环节闭合的轻量级在线方案，是继 Living-Harness（离线经验演化）之后规划层故障恢复的最新在线对应工作——前者离线积累修复知识，本文在线触发即时修复，二者互补。
