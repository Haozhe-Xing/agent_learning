...
本书共 25 章、6 个部分，章与章之间有明确的依赖关系。下面这张全景图展示了全书的结构主干与三条推荐路线——**节点左边框的色条标记它属于哪条路线**（可同时属于多条）。
...
- 🟠 **快速上手（5 章，适合产品经理 / 管理者）**：`1 → 3 → 8 → 12 → 23`
  先建立 Agent 直觉（第1章），了解工具调用（第3章），掌握 Harness 工程规范（第8章），用一个主流框架（第12章 LangChain）做出第一个编程助手项目（第23章）。跳过深理论，最快见到成果。

- 🔵 **工程师路线（11 章，适合想系统掌握开发的工程师）**：`1 → 2 → 3 → 4 → 5 → 7 → 12 → 13 → 16 → 22 → 23`
  在快速线基础上补齐 LLM 基础（第2章）、记忆与规划（第4、5章）、上下文工程（第7章），再学 LangChain + LangGraph + Claude Code 三套框架，最后落到部署（第22章）与实战项目（第23章）。

- 🟣 **研究者路线（8 章，适合深入理论与前沿的研究者）**：`1 → 2 → 4 → 5 → 10 → 11 → 18 → 20`
  聚焦原理与前沿：LLM 基础（第2章）、记忆与规划理论（第4、5章），深入 Agentic-RL 训练（第10章）与自我进化（第11章），再扩展到多 Agent 系统（第18章）与评估方法（第20章）。

> 💡 **阅读建议**：实线箭头表示"部分内顺序"，虚线箭头表示"跨部分主干衔接"。无论走哪条路线，都建议先读第1章建立全局观；综合项目篇（第23-25章）建议放在最后，作为所学知识的整合检验。
