...
> 🧬 **一句话**：把文件系统操作提升为一等记忆动作，双闭环（外审查 + 内学习）自动改进记忆结构，32B 开源模型三游戏性能提升 2–4×。

![AutoMem：将记忆管理作为认知技能自动化学习 论文核心图](../svg/chapter_memory_paper_2607.01224.png)

> 图源：AutoMem：将记忆管理作为认知技能自动化学习 论文（来源：2026, arXiv:2607.01224）
...
> 🧬 **一句话**：解决「幽灵记忆」（旧/当前/过渡事实混存），状态感知叠加层显式暴露时态标签，冲突准确率 +0.240。

![A-TMA：长期 Agent 记忆的状态感知失效解耦 论文核心图](../svg/chapter_memory_paper_2607.01935.png)

> 图源：A-TMA：长期 Agent 记忆的状态感知失效解耦 论文（来源：2026, arXiv:2607.01935）
...
> 🧬 **一句话**：主动记忆干预：独立记忆 Agent 在关键时刻注入提醒而非被动检索，Terminal-Bench +8.3pp、τ2-Bench +6.8pp。

![Remember When It Matters：主动记忆干预 论文核心图](../svg/chapter_memory_paper_2607.08716.png)

> 图源：Remember When It Matters：主动记忆干预 论文（来源：2026, arXiv:2607.08716）
...
### [超认知模式 SCM：面向 Agent 记忆的路由架构（2026）](https://arxiv.org/abs/2607.19096)

**发表**：2026 年 7 月 21 日 | [arXiv:2607.19096](https://arxiv.org/abs/2607.19096)

> 🧬 **一句话**：按查询类型路由到专用检索-合成路径（直接查询/关系推理/长史综合），三基准竞争性成绩，取代「一刀切」语义检索。

![超认知模式 SCM：面向 Agent 记忆的路由架构 论文核心图](../svg/chapter_memory_paper_2607.19096.svg)

> 图源：超认知模式 SCM：面向 Agent 记忆的路由架构 论文（来源：2026, arXiv:2607.19096）

**核心贡献**：Agent 记忆负载天然混杂三类工作：直接事实查询、关系链/当前状态推理、长历史综合。SCM（Supra Cognitive Modes）架构将每个查询通过冻结的语义分类器和运行时门控路由到对应的检索-合成载荷，三条路径分别对应融合词法与稠密检索的直接查询、图或多跳迭代推理、分层长篇合成，底层共享多粒度嵌入 + 三元组提取 + 事实版本元数据的统一摄取基础设施。在 LoCoMo、MemoryAgentBench、LongMemEval 三个基准上，系统分别达到 84.87%（事实问答）、61.49%、86.00% 的竞争性成绩，每类查询均由专用路径处理而非统一检索。

**与本章关系**：对应本章「记忆的检索与使用」知识点，SCM 以显式路由取代"一刀切"语义检索，是继 AutoMem（记忆作为技能）、A-TMA（时态状态解耦）之后记忆架构精细化的最新方向，将记忆系统的认知工作负载分类问题转化为可工程化的路由决策。

---

### [PRO-LONG：以程序化记忆赋能长时程推理（2026）](https://arxiv.org/abs/2607.20064)

**发表**：2026 年 7 月 22 日 | [arXiv:2607.20064](https://arxiv.org/abs/2607.20064)

> 🧬 **一句话**：程序化记忆 = 完整交互日志 + 代码搜索，无损记忆与精准检索并存，ARC-AGI-3 提升 18.0pp、token 少 4.2–5.8×。

**核心贡献**：长时程任务中，"保存更多信息"与"检索更精准"存在天然矛盾——全量保留使相关细节检索变难，压缩又损失关键状态。PRO-LONG 提出以**程序化记忆**作为 LLM Agent 的上下文管理基础：维护一份完整、结构化的交互日志，并借助代码 Agent 的代码执行能力高效搜索历史。该设计做到了"无损记忆"与"精准检索"并存：在完整 ARC-AGI-3 公开游戏集上，平均比基础代码 Agent 提升 **18.0 个百分点**，以 4.2–5.8× 更少 token 匹配或超越现有最优专用框架（pass@1 高达 76.1%）；配合 Fable 5，best@2 达 **97.4%**，总成本仅 1750 美元。

**与本章关系**：直接对应本章「记忆的存储与检索」与「长期记忆管理」知识点，PRO-LONG 以"程序化索引"取代语义检索，将长期记忆的结构优势与代码搜索效率结合，是继 AutoMem（记忆作为技能）和主动记忆干预之后，长时程 Agent 记忆管理的最新架构突破。

---

### [在职学习：冻结权重 Agent 从部署反馈中持续学习（2026）](https://arxiv.org/abs/2607.22157)

**发表**：2026 年 7 月 24 日 | [arXiv:2607.22157](https://arxiv.org/abs/2607.22157)

> 🧬 **一句话**：冻结权重 Agent 靠外部记忆把每次情节蒸馏为可检索规则，结果反馈 1.6×、纠错 2.6×，记忆可跨模型迁移。

![在职学习：冻结权重 Agent 的持续学习 论文核心图](../svg/chapter_memory_paper_2607.22157.svg)

> 图源：在职学习：冻结权重 Agent 的持续学习 论文（来源：2026, arXiv:2607.22157）

**核心贡献**：AI Agent 在每次运行中都会遇到学习机会，但由于模型在部署时权重冻结，今天解决的难题明天从零开始——这是当前生产 Agent 系统的通用痛点。本文证明，当冻结模型配以**将每次情节蒸馏为可检索自然语言规则的外部记忆**时，日常运行产生的反馈信号即足以支持持续学习。在 τ-bench 银行领域，从结果反馈（一位二进制判决）学习使单次成功率提升至基线的 **1.6×**，从错误修正学习则达 **2.6×**，解决了 84 个从未被基线解决的任务中的 22 个；结果在 Mistral Large（本地部署）与 Claude Sonnet 5 两个模型上可复现，且一个模型的记忆库迁移到另一个模型后，接收方仍高于自身无记忆基线，证明记忆的跨模型可迁移性。

**与本章关系**：直接对应本章「长期记忆的写入与更新」知识点，提出了一种无需微调的持续学习闭环——运行时外部记忆作为"可演化知识库"，填补了 MemGPT/AutoMem 等框架在"从失败和修正中自动更新记忆"方向的空白，对数据主权敏感的生产部署场景（本地开源模型）尤具参考价值。

---

### [图原生双时态记忆存储：面向对话 AI Agent 的持久记忆架构（2026）](https://arxiv.org/abs/2607.26520)

**发表**：2026 年 7 月 29 日 | [arXiv:2607.26520](https://arxiv.org/abs/2607.26520)

> 🧬 **一句话**：Neo4j 属性图 + 双时态数据模型（有效时间 + 事务时间），支持任意时间点检索而不物理覆盖历史。

![图原生双时态记忆存储 论文核心图](../svg/chapter_memory_paper_2607.26520.svg)

> 图源：图原生双时态记忆存储 论文（来源：2026, arXiv:2607.26520）

**核心贡献**：对话 AI Agent 普遍缺乏跨会话持久记忆，全量注入聊天历史会耗尽上下文窗口，而依赖第三方记忆服务又带来数据隐私风险。本文提出基于 Neo4j 属性图 + HNSW 向量索引 + **完整双时态数据模型**的 Agent 本地记忆存储：每条记忆以不可变身份节点 + 版本化内容节点保存，携带**有效时间**（事实在世界中为真的时间段）和**事务时间**（数据库记录该事实的时间段）两个时间区间，支持任意时间点的语义检索而不物理覆盖历史。语义边在写入时通过余弦相似度自动维护。在 LongMemEval（500 题）上，当前状态语义路径整体 R@10 达 46.7%，知识更新题达 80%；时间旅行路径在知识更新题同样达 80%，但在时序推理题上召回降低，指向具体设计改进方向。

**与本章关系**：直接对应本章「持久记忆存储」与「情节记忆」知识点，双时态模型将"有效时态"与"事务时态"解耦，在不物理删除旧记录的前提下支持历史回溯查询，是继 A-TMA（时态状态解耦）之后对 Agent 记忆时序一致性问题的最新工程实现，对需要长期记忆且无法依赖云端记忆服务的本地部署 Agent 具有直接参考价值。

---

### [基于文件系统的 LLM Agent 记忆：组织、演化与可持续性（2026）](https://arxiv.org/abs/2607.26637)

**发表**：2026 年 7 月 30 日 | [arXiv:2607.26637](https://arxiv.org/abs/2607.26637)

> 🧬 **一句话**：系统研究文件系统记忆的完整生命周期（组织/演化/可持续），指出「主动清理合并」优于「无限追加」。

![基于文件系统的 LLM Agent 记忆 论文核心图](../svg/chapter_memory_paper_2607.26637.png)

> 图源：基于文件系统的 LLM Agent 记忆 论文（来源：2026, arXiv:2607.26637）

**核心贡献**：本文系统研究了以文件系统作为 LLM Agent 长期记忆后端的完整生命周期——从初始存储组织方式，到记忆随任务演化的更新策略，再到超长时间运行后的可持续性挑战。论文指出记忆后端的选择会实质性影响系统的可审计性与成本：文件系统结构在可读性和可检索性之间存在基本张力；随着记忆体积增长，无结构扁平存储会导致检索退化，而过度层次化则增加导航开销；可持续性核心在于主动清理与合并过时记忆，而非无限追加。论文还分析了文件系统记忆在安全性（提示注入中间文件）和隐私（持久化敏感内容）方面的特殊风险，并提出最佳实践框架。

**与本章关系**：直接对应本章「记忆的存储结构」与「长期记忆管理」知识点，是继 PRO-LONG（程序化记忆索引）和 AutoMem（记忆作为技能）之后对文件系统原生记忆方案最系统的工程评估，为选型文件系统型记忆后端的 Agent 工程师提供了全面的架构参考。

---

### [Setoka：异构数据上个性化 Agent 分层用户理解基准（2026）](https://arxiv.org/abs/2607.27056)

**发表**：2026 年 7 月 29 日 | [arXiv:2607.27056](https://arxiv.org/abs/2607.27056)

> 🧬 **一句话**：四层用户理解基准（语义/情节/行为/人格），揭示现有系统在行为模式与人格抽象上的系统性盲区。

![Setoka：分层用户理解基准 论文核心图](../svg/chapter_memory_paper_2607.27056.png)

> 图源：Setoka：分层用户理解基准 论文（来源：2026, arXiv:2607.27056）

**核心贡献**：现有记忆基准主要评估 Agent 能否检索对话历史中明确陈述的事实，未能评估更深层的用户理解。Setoka 基于认知与人格心理学理论，定义**四层用户理解层次**：语义记忆（明确事实检索）、情节记忆（特定事件召回）、行为模式（行为规律推断）和人格特质（跨时间跨来源的高层抽象）。配套设计了基于心理测量学的合成异构用户数据与查询生成管线（保护隐私的同时保持真实性）。评估 3 个语言模型×5 种记忆系统×10 个合成用户，发现：现有系统在语义记忆检索上表现良好，但情节记忆性能明显下滑，行为模式和人格特质理解（需整合跨时间、跨来源的碎片化信息）性能进一步下降。**结论：用户理解不能靠简单事实检索解决，需要专为跨源整合与长期行为抽象设计的记忆机制**。

**与本章关系**：直接对应本章「情节记忆」与「个性化 Agent 记忆」知识点，Setoka 揭示了当前记忆系统在"用户理解深度"维度的系统性盲区——从事实存储到人格建模是一个四级递进的挑战，为下一代个性化 Agent 记忆系统的设计提供了明确的评测框架与研究方向。

---

### [当记忆变为权威：记忆整合边界处的权威崩溃基准测试（2026）](https://arxiv.org/abs/2608.01679)

**发表**：2026 年 8 月 4 日 | [arXiv:2608.01679](https://arxiv.org/abs/2608.01679)

> 🧬 **一句话**：揭示「权威崩溃」：记忆整合丢失来源权威性，48/49 配置中崩溃、后续 50.3% 未授权动作率。

**核心贡献**：持久化Agent 在运行中会将外部信息整合进长期记忆，而本文揭示了一种关键失效模式——**权威崩溃（Authority Collapse）**：记忆内容被保留下来，但其来源的权威性（"谁说的"、"此人是否有权触发这条指令"）在整合过程中丢失，导致 Agent 后续依据无授权来源的记忆内容执行高权限动作。论文构建了针对此问题的系统性基准，在 49 种配置下进行评估，发现 **48 种配置中存在权威崩溃**；被洗掉来源信息的记忆（washed memories）在后续会导致 **50.3% 的未授权动作率**。研究还给出了三类缓解策略：来源标注保留、整合时的权限过滤、记忆访问时的授权重验证，并量化了各策略的防护效果。

**与本章关系**：直接对应本章「持久记忆管理」与「记忆安全」知识点，权威崩溃是持久 Agent 记忆系统的一类全新失效模式，不同于已收录的 Salami攻击（跨会话串谋投毒）——后者是攻击者主动注入恶意内容，本文揭示的是无攻击者干预情况下整合流程自身导致的授权失控，对设计生产级长期记忆架构具有直接安全影响。

---

### [QUMem：查询条件用户状态推断的个性化记忆框架（2026）](https://arxiv.org/abs/2608.16168)

**发表**：2026 年 8 月 17 日 | [arXiv:2608.16168](https://arxiv.org/abs/2608.16168)

**核心贡献**：LLM Agent 的个性化需求要求从长期演化的交互历史中准确理解用户偏好，但现有系统面临三重局限：基于固定轮数/token 数/会话的边界会混入不相关对话或割裂事件与其决策；将同一次交互的多条用户信息合并存储导致无法独立检索；单次 top-k 检索查询无法联合捕获偏好演化、时效性和上下文适用性。QUMem 提出**查询条件用户状态推断**框架：先将历史按语义连续性切分为可变长度情节，再将每个情节分解为可独立检索的事实记忆、偏好记忆和可迁移洞察三类；推理时由三步序列化 Agent 识别信息需求、多查询检索及联合推断时效与上下文均有效的用户状态。在 PersonaMem 和 KnowU-Bench 上达到 SOTA。

**与本章关系**：直接对应本章「长期记忆」与「个性化 Agent 记忆」知识点，QUMem 的"查询条件用户状态推断"范式将记忆检索从"找相关片段"升级为"推断当前任务下的用户当前状态"，与已收录的 Setoka（四层用户理解基准）形成互补——Setoka 定义问题空间，QUMem 提供解决框架。

---

### [CAMA：多 Agent 记忆相关性偏置的溯源去耦仲裁框架（2026）](https://arxiv.org/abs/2608.19701)

**发表**：2026 年 8 月 20 日 | [arXiv:2608.19701](https://arxiv.org/abs/2608.19701)

**核心贡献**：多 Agent 系统中不同 Agent 写入长期记忆时可能继承同一上游信息来源，现有记忆方法将检索到的记忆当作独立证据处理——该独立性假设在多 Agent 场景下系统性失效，导致相关证据被重复计数（"虚假多数"）。CAMA（记忆仲裁框架）将记忆建模为查询条件的证据组，用**神经依赖推断 + 溯源符号先验**联合估计有效独立来源数量，再学一个顺序恢复策略主动补检初始检索集中缺失的独立证据，从结构上抑制虚假多数带来的错误仲裁。在多 Agent 记忆仲裁任务上验证了 CAMA 对虚假多数的显著抑制效果。

**与本章关系**：直接对应本章「多 Agent 共享记忆」与「记忆可信度评估」知识点，CAMA 首次将"记忆来源独立性"建模为可估计量，揭示了多 Agent 场景下记忆污染的系统性根源——与已收录的 SCM 超认知路由（多类工作负载记忆路径）和 A-TMA（时态状态感知解耦）共同构成多 Agent 记忆系统的可靠性方法图谱。

---
### [Recuris：递归经验-工作记忆演化，让长 Horizon Agent 自我修复技能库（2026）](https://arxiv.org/abs/2608.24876)

**发表**：2026 年 8 月 25 日 | [arXiv:2608.24876](https://arxiv.org/abs/2608.24876)

**核心贡献**：长 Horizon 任务中，聊天记录不断堆积导致早期目标被淹没，检索到的技能也容易与当前状态错位。Recuris（NUS/Stanford/Oxford/Princeton）提出**工作记忆（Working Memory）与经验记忆（Experiential Memory）解耦**的架构：工作记忆持续记录当前任务目标和已验证进展，经验记忆以工作状态为条件检索技能；执行轨迹被结构化为可定位的失败证据，由固定 Meta-Agent 做局部的、验证门控的技能库更新，形成有界递归记忆演化闭环。在 4 个长 Horizon 基准、10 个模型上的 37 组对比中，35 组任务成功率上升；GPT-5.6 Sol 在 τ²-Retail 提升 +17.8 点，Claude Opus 5 提升 +15.6 点至 87.9%，最长任务分组提升 +32.2 点。

**与本章关系**：直接对应本章「情景记忆」与「长期记忆动态演化」知识点，Recuris 的"递归"指失败证据反向驱动技能库局部更新——这是区别于传统"追加即遗忘"式经验积累的关键创新，"验证门控"确保演化有界，避免自我改写失控，是目前长 Horizon Agent 记忆架构的 SOTA 方案。

---

### [HiPS：分层个性化策略，让 Agent 记忆管理兼顾共性规律与用户特异性（2026）](https://arxiv.org/abs/2608.25329)

**发表**：2026 年 8 月 26 日 | [arXiv:2608.25329](https://arxiv.org/abs/2608.25329)

**核心贡献**：记忆增强 Agent 在长对话中维护用户画像，但最优记忆决策（保留/压缩/丢弃什么）是用户特异的且随策略优化动态演变——现有方法在训练前就固定一刀切策略。HiPS（EMNLP 2026 Main）提出**分层个性化策略**：用 Universal Strategy 从跨 persona 轨迹中提炼共享原则，用 Persona Delta Distillation 为行为偏离全局模式的用户生成个性化规则；Cross-Level Rule Flow 动态调整两层边界——验证充分的个性化规则晋升，与全局规则冲突的降级。整个演化闭环以任务成果为锚点，防止规则脱离实际效果漂移。

**与本章关系**：直接对应本章「个性化记忆管理」与「记忆策略优化」知识点，HiPS 将"记忆管理策略"本身作为可学习对象，其全局策略+用户特异delta的分层架构，与已收录的 QUMem（查询条件用户状态推断）形成互补——QUMem 优化"检索什么"，HiPS 优化"保留/丢弃什么"。

---

### [EARM：检索器该有记忆——经验摊销重排序降低 Agent 长期记忆检索成本（2026）](https://arxiv.org/abs/2608.22767)

**发表**：2026 年 8 月 28 日 | [arXiv:2608.22767](https://arxiv.org/abs/2608.22767)

**核心贡献**：长期 Agent 会累积大量记忆，检索器通常是无状态的：语义向量检索快但不精准，LLM 重排序精准但每次都需要对大批候选重新打分，历史分数用完即丢。EARM 将每次查询-记忆配对的 LLM 相关性分数存入持续累积的稀疏矩阵，用**因果矩阵补全**学出矩阵的共享结构，再用少量新打分数加估计分数联合排序其余候选。在只需直接评分 17.5% 候选的情况下，答案准确率仍比纯语义检索最多提升 6.62%。核心思路：重复被查询的记忆库中，历史打分结果本身就是可复用的结构化信号，而不是一次性消耗品。

**与本章关系**：直接对应本章「长期记忆检索效率」与「记忆系统成本优化」知识点，EARM 将推荐系统的协同过滤思想迁移到 Agent 记忆检索领域，揭示了"检索经验可摊销"这一被长期忽视的记忆系统优化维度，与已收录的 Scroll（上下文即环境）和 Recuris（递归技能库更新）共同构成高效长期记忆系统的方法图谱。

---

### [Agent 对陈旧记忆的盲目信任：持久记忆系统的过信任问题（2026）](https://arxiv.org/abs/2609.01852)

**发表**：2026 年 9 月 1 日 | [arXiv:2609.01852](https://arxiv.org/abs/2609.01852)

**核心贡献**：当 Agent 的持久记忆中存储了已过时的事实时，模型是否会识别并质疑旧记忆？本文对 Qwen3 系列（0.6B~8B）进行系统测试，结论令人警惕：在"需要调用记忆"的场景中，各规模模型无一例外地选择信任陈旧存储值，信任率高达 92%~100%。更反直觉的发现是：将旧记忆伪装得"看起来更新"（添加更近的时间戳），反而让更大的模型（4B/8B）更容易被欺骗——模型能力越强越信任"权威性标注"，越难识破表面新鲜度的伪装。对 4B/8B 模型，加入真实时间戳和来源元数据可大幅恢复准确率；较小模型（0.6B/1.7B）则需在输入前预先解决记忆冲突。

**与本章关系**：直接对应本章「记忆可靠性」与「长期记忆的真实性验证」知识点，本文揭示了 Agent 持久记忆系统被忽视的安全隐患——记忆应被当作"不可信输入"而非"可靠事实"来处理；"大模型更难识破伪装新鲜度"这一反直觉发现，对长期运行 Agent 的记忆设计原则有直接影响，与已收录的 CAMA（虚假多数问题）共同揭示记忆可信度的多维挑战。

---

### [MemForest：事件树划分与渐进合并实现高效智能体记忆管理（2026）](https://arxiv.org/abs/2609.08273)

**发表**：2026 年 9 月 8 日 | [arXiv:2609.08273](https://arxiv.org/abs/2609.08273)

**核心贡献**：上海交通大学、阿里巴巴、清华大学等联合提出 MemForest，将 Agent 历史记忆按事件组织成树状结构，并通过渐进式合并重复/冗余节点。在 M3-Agent 框架的两项多模态基准上：压缩 **50% 记忆**后仍保留 **99.7%** 任务表现，检索速度加速 **2.24 倍**。核心思路是用事件树的层级组织替代平铺的记忆列表——相关事件自然聚合为子树，检索时可直接定位到事件分支而非扫描全部记忆，渐进合并同类节点则持续压缩冗余存储。

**与本章关系**：直接对应本章「长期记忆效率」与「记忆组织结构」知识点，MemForest 展示了将树形数据结构思想迁移到 Agent 记忆管理的系统性收益——事件树的层次组织同时解决了"检索效率"和"存储冗余"两个问题，是继 EARM（经验摊销重排序）和 HiPS（分层个性化策略）之后又一从工程角度大幅降低长期记忆成本的新方案。

---

### [MERIT：面向工具使用 Agent 的长期记忆成本感知评测基准（2026）](https://arxiv.org/abs/2609.05441)

**发表**：2026 年 9 月 5 日 | [arXiv:2609.05441](https://arxiv.org/abs/2609.05441)

**核心贡献**：现有长期记忆评测（LoCoMo、LongMemEval）测的是对话历史问答，而非记忆事实是否真正改变工具使用 Agent 的行为。MERIT（Memory Evaluation for Realistic Instrumented Tasks）以任务执行中的边际效用为核心指标：提供三个领域的情节式工具使用任务，通过自动泄漏检查验证任务对先前情节的实际依赖性，并对每次记忆操作做完整 token 和美元计费。跨 23,440 个评分情节（总花费 \$42.57）的实验发现：记忆帮助最显著的场景是跨会话更新事实任务；而在多数任务上，记忆操作的成本使得净效益为负——"记忆有用吗"取决于具体任务场景，而非一个通用结论。

**与本章关系**：直接对应本章「记忆系统评测」与「长期记忆的实用价值」知识点，MERIT 将记忆评测从对话召回转向任务执行中的因果影响分析，并引入成本核算维度——"记忆是否值得"取决于其在真实任务中的净效益，是评估 Agent 记忆系统价值时不可缺少的方法论修正，与已收录的 MemForest（记忆压缩效率）形成互补，共同指向记忆系统的"质量-成本"权衡。

---

### [RSM-full：语义分组写入与整组检索，在严格 Token 预算下保留 83% 全量记忆质量（2026）](https://arxiv.org/abs/2609.04915)

**发表**：2026 年 9 月 6 日 | [arXiv:2609.04915](https://arxiv.org/abs/2609.04915)

**核心贡献**：长时程 LLM Agent 在严格 Token 预算（约 4k）下面临两难：全量历史塞不进上下文，孤立 chunk 检索则丢失语义关联。RSM-full（Jiahe Geng、Jinpeng Wang、Kun Yuan）提出**在线聚合记忆流水线**：① 余弦门控最大成员合并写入规则——新记忆到来时与最相近记忆簇合并，相似度低于阈值才开新簇；② 原子感知分组上下文打包——检索时以完整记忆组为单位填充 token 窗口，而非逐条 chunk 评分再拼接。在 AMA-Bench（约 4k prompt token 预算）上，RSM-full 以 **32% 的 token 消耗**保留了全量历史记忆 **83%** 的质量，比 Online K-Means 提升 +3.5~6.0pp（p<0.001）；在独立 RealMem 基准上同样显著超越 Streaming-Proto 和 Budget-RAG，与 BM25-RAG 相当。消融显示大部分增益来自合并写入规则（+5.7pp）和分组打包（+5.0pp）。

**与本章关系**：直接对应本章「长期记忆检索效率」与「Token 预算下的记忆管理」知识点，RSM-full 的核心洞见是"先整理再检索"：把相关记忆分组写入，检索时以组为粒度而非逐条 chunk 评分，同时解决了相同实体多称谓导致的碎片化问题——与已收录的 EARM（历史打分摊销）和 MemForest（事件树压缩）共同构成高效长期记忆系统的方法全景。

---

### [模型升级等于记忆迁移：自由格式笔记跨模型继承精度下降 13 个百分点（2026）](https://arxiv.org/abs/2609.05339)

**发表**：2026 年 9 月 4 日 | [arXiv:2609.05339](https://arxiv.org/abs/2609.05339)

**核心贡献**：LinkedIn 工程师 Ankit Goyal 和 Jaideep Ray 的受控交换实验，回答了一个被工程师普遍忽视的问题：当 Agent 底层模型从 A 升级到 B 时，A 写入的记忆能被 B 正确使用吗？实验保持 48 条对话历史不变，只换记忆的读写模型（Llama-3.1-8B-Instruct ↔ Qwen2.5-7B-Instruct），测量精确匹配准确率。**核心发现**：固定模式字段（KG-fixed）跨模型继承几乎无损（±0.0004）；自由文本笔记（NOTES）继承精度在一个方向下降 **13.28pp**，因为原写入者已经遗漏的事实，改写者无法凭空恢复。Embedding 升级（bge-large-en v1.0 → v1.5）若用 50/50 混合索引，只能取回全量重新嵌入收益的 **41.7%**（+4.96pp vs. +11.90pp）；"改写旧记忆存储"永远无法在任何一条历史上恢复到新模型自写水平。

**与本章关系**：直接对应本章「记忆可靠性」与「记忆系统生命周期管理」知识点，本文将"模型版本升级"正式定义为一次记忆迁移事件，给出了三条工程守则：优先结构化字段存储（跨模型等价）、模型切换时全量重建 Embedding 索引（不要用混合索引）、保留原始历史记录（是能恢复精度的唯一手段）；与已收录的「Agent 对陈旧记忆的盲目信任」共同构成 Agent 记忆系统长期运维的安全底线。

---

### [持续学习机制的可组合性：三锚点叠加实现长视野记忆（2026）](https://arxiv.org/abs/2609.06986)

**发表**：2026 年 9 月 9 日 | [arXiv:2609.06986](https://arxiv.org/abs/2609.06986)

**核心贡献**：约翰斯·霍普金斯大学（Alvin Zhang、Daniel Khashabi、Tianmin Shu）研究了 LLM 在 100 个任务上连续微调后的记忆保留问题：天真的顺序微调记忆保留率仅 **1.2%**。论文沿两个设计维度系统探索组合方案——"锚点"（保留什么：数据重放/自蒸馏/权重正则化）× "低秩分配"（更新存储位置：共享/合并 LoRA），并用任务级逐步减半（successive halving）搜索组合空间。最佳组合（三锚点全开 + 合并 LoRA）将记忆保留率提升至 **34.9%**，比基线改善 **28 倍**；关键发现是数据锚点与合并 LoRA 存在**超加性交互**——合并收益超出两者独立贡献之和，在所有三个测试数据集上一致成立。

**与本章关系**：直接对应本章「长期记忆的持久化」与「记忆遗忘与更新机制」知识点，100 任务顺序学习设置与长期运行 Agent 经历的"任务流"在结构上等价；超加性组合结果打破了"单一机制足够"的既有预设，为构建真正不遗忘的 Agent 记忆系统提供了可直接引用的组合原则。

---

### [AutoViewMem：对话长时记忆的自配置正交视图框架（2026）](https://arxiv.org/abs/2609.21940)

**发表**：2026 年 9 月 21 日 | [arXiv:2609.21940](https://arxiv.org/abs/2609.21940)

**核心贡献**：现有 Agent 记忆系统依赖固定粒度或静态 schema，导致"偏好、事件、约束、时序更新"等异构信息混存时的 top-K 检索噪声高、相关证据排名差。AutoViewMem 提出**写入时语义解耦**的思路：从交互轨迹中自动发现候选语义视图，选出低重叠的紧凑视图集，并在写入时对每条记忆做视图感知的结构化抽取（含来源溯源）——把语义解耦从检索时移到写入时，使标准 top-K 相似度搜索无需显式路由或迭代检索即可拉回聚焦证据。在 LoCoMo 和 PersonaMem 基准上（Qwen3-8B / Qwen3-14B），AutoViewMem 在长视野问答和个性化两项指标上均超越强记忆基线，同时保持简洁推理流水线。

**与本章关系**：直接对应本章「记忆表示与检索」与「长期对话记忆」知识点，AutoViewMem 将"记忆 schema 设计"重新定义为数据驱动的自动配置问题——不需要人工预设视图类型，系统从真实交互中学习"什么粒度的视图对这批对话最有检索价值"；与已收录的 MemForest（事件树结构）构成对偶：MemForest 侧重时序层次组织，AutoViewMem 侧重语义正交性与写入时解耦。

---

### [MDL：基于三信号互补的可解释记忆决策控制器（2026）](https://arxiv.org/abs/2609.22043)

**发表**：2026 年 9 月 22 日 | [arXiv:2609.22043](https://arxiv.org/abs/2609.22043)

**核心贡献**：现有记忆系统专注于高效检索，却忽视了一个更基础的问题——检索到的记忆是否值得被信任？当记忆库存在冲突信息时，标准 RAG 盲目注入记忆会放大幻觉：在易受记忆注入影响的模型上，冲突记忆下的幻觉率远高于无记忆基线。受前额叶皮层记忆信号机制启发，论文提出 **MDL（Memory Decision Layer）**：一个零参数记忆决策控制器，位于检索与生成之间。其核心是**三信号互补编码器**，通过 QR 正交子空间投影，将相关性、可靠性和任务风险三路信号融合为可解释的决策表示，量化检索记忆的可信度。MDL 显式解耦置信度与一致性，引入风险反转与显式弃答机制。实验证明 MDL 在冲突记忆场景下将幻觉率降低约 **56.04%**，高风险场景接近零幻觉；全白盒设计，每次决策仅需 **0.14 ms**，比 Embedding 检索步骤快 50 倍。

**与本章关系**：直接对应本章「记忆可靠性」与「检索增强生成的幻觉问题」知识点，MDL 将"是否使用检索结果"定义为一个需要专门控制器的决策问题——仅有检索层不够，记忆与生成之间还需要可信度门控；与已收录的 AutoViewMem（写入时语义解耦）和 MemForest（事件树压缩）共同构成记忆系统的"写入→存储→信任决策"完整链路。

---

### [Jev-Mem：System-One 轻量控制器驱动的高效 Agent 记忆系统（2026）](https://arxiv.org/abs/2609.23986)

**发表**：2026 年 9 月 21 日 | [arXiv:2609.23986](https://arxiv.org/abs/2609.23986)

**核心贡献**：德克萨斯大学达拉斯分校团队（Jiang、Li、Li）指出，现有记忆系统将"分类、路由、评分、何时停止检索"等高频但范围明确的决策全部交由生成式 LLM 执行，导致长时运行 Agent 的记忆模块成本和延迟急剧膨胀。Jev-Mem 借用认知科学的 System One/Two 框架，将这些结构化低歧义决策拆给不需要文本生成的 System-One 轻量控制器处理，只有开放式推理与最终答案生成才调用 System-Two LLM。在 LoCoMo 长对话记忆基准上，Jev-Mem 整体 LLM-as-a-Judge 分数达 **0.777**（比最强基线高 **11.0%**），记忆构建时间缩短至 **158 秒**（比最快基线快 **6.6 倍**），平均查询延迟降至 **0.93 秒**（下降 **36.7%**）；代码已公开发布（github.com/libingzheren/Jev-Mem）。

**与本章关系**：直接对应本章「记忆检索效率」与「记忆系统架构设计」知识点，Jev-Mem 首次将"记忆系统要多智能"与"每个决策都需要生成式 LLM"这两件事拆分——效能与效率可同时提升无需相互牺牲；与已收录的 AutoViewMem（写入时语义解耦）和 MDL（可信度门控）共同构成"记忆写入→存储→高效查询"的完整优化图谱。

---

### [MoM：提交当前值同时保留溯源的 Provenant Memory 设计（2026）](https://arxiv.org/abs/2609.25054)

**发表**：2026 年 9 月 22 日 | [arXiv:2609.25054](https://arxiv.org/abs/2609.25054)

**核心贡献**：长时程 LLM Agent 的记忆核心问题是"什么当前有效"，而非"曾经记录了什么"。本文指出现有设计的两极困境：检索型记忆在查询时重建当前值（过时值反复进入、同一冲突反复裁决）；CRUD 写入型记忆直接覆写（错误更新不可恢复）。Memory of Memory（MoM）取其中间——提交当前值的同时保留被替换值，实例化为 **Provenant Memory（P-Mem）**：一种类型化溯源图，活跃前沿为每个已解析 key 暴露唯一当前值，被替换旧值保留为溯源节点，类型化操作决定新观测的语义角色（支持/取代/争议/撤销/解决）。在修订链上 P-Mem 保持 **100%** 准确率（查询时读取仅 25%）；图引导剪枝将知识更新过时应答率从 **19.4%** 降至 **10.9%**；已提交错误的回溯修复率 **100%**（CRUD 记忆为 0%）；读取 token 减少约 **4 倍**。

**与本章关系**：直接对应本章「长期记忆的一致性维护」与「记忆更新与错误恢复」知识点，P-Mem 将"记忆不仅存内容，还存每条记忆的溯源与历史"形式化，解决了现有写入型记忆无法从错误更新中恢复的核心缺陷；与已收录的 MDL（可信度门控）、AutoViewMem（写入时解耦）、Jev-Mem（查询效率）共同构成本章记忆系统"写入→可靠性→高效查询"的四维优化体系。

---

### [SpeakerMem-R1：面向多方对话的以说话者为中心的双轨记忆（2026）](https://arxiv.org/abs/2609.26780)

**发表**：2026 年 9 月 22 日 | [arXiv:2609.26780](https://arxiv.org/abs/2609.26780)

**核心贡献**：浙江大学团队针对多方对话（群聊、会议、社交场景）中的长期记忆问题提出 SpeakerMem-R1。现有通用 LLM 记忆系统在多方设置下有两个核心瓶颈：**消息归因**（谁说了什么、说的是谁）和**跨成员状态重建**（跨时间线整合分散线索）。SpeakerMem-R1 的双轨记忆同时存储：①以说话者标注的逐字消息（保留原始表达精度）；②按人物/群组视图组织的衍生状态（支持关系变化追踪）。查询时按实体、事件、时间跨两轨融合证据。为减少结构化记忆写入时的归因错误，使用 **SpeakerLevenshtein 距离 + 说话者条件 GRPO** 训练 Writer-R1（RL 将 SFT 基线准确率从 57.38% 提升至 68.20%）。在 GroupMemBench / SocialMemBench / EverMemBench 上分别达 47.9% / 69.2% / 62.33%，EverMemBench 公开榜单 SOTA；LoCoMo 1986 题全题集 70.85%。

**与本章关系**：直接对应本章「情景记忆」与「长期对话记忆」知识点，SpeakerMem-R1 将多方对话场景的特殊挑战（多说话者归因、群体关系演化）显式编码到记忆架构中——提示现有把记忆当"单说话者日志"的系统在多方场景会系统性失效；双轨设计也为"如何在结构化摘要与原文保真度之间取得平衡"这一记忆工程核心矛盾提供了一种可操作的解决路径。

---

### [即时记忆（JIT Memory）：延迟裁剪到读取时刻，按任务需求自适应构建记忆载荷（2026）](https://arxiv.org/abs/2609.27334)

**发表**：2026 年 9 月 24 日 | [arXiv:2609.27334](https://arxiv.org/abs/2609.27334)

**核心贡献**：Salesforce Research 与新加坡国立大学团队（Zhou、Li、Liu 等）提出 **Just-in-Time Memory（JIT Memory）**：保留原始 Agent 轨迹，将记忆裁剪推迟到**读取时刻**，由当前任务决定紧凑、相关的记忆载荷，而非在写入时就对记忆进行压缩或摘要。核心洞察：写入时的裁剪不可避免地丢失未来任务所需的信息，而读取时的按需裁剪可根据当前上下文精准选取。在 ALFWorld、WebShop 和 τ²-bench 三个基准上，JIT Memory 相比学习型和启发式写入时记忆方法最多提升 **+16.3 个绝对成功率百分点**；在跨任务泛化场景下优势尤为突出。

**与本章关系**：直接对应本章「记忆写入策略」与「长期记忆利用效率」知识点，JIT Memory 提出了一个反直觉但经实验验证的设计原则——"不要急于裁剪记忆，留到用时再决定选什么"；与已收录的 AutoViewMem（写入时语义解耦）形成核心对比：两者代表记忆管理的两种思路，为设计 Agent 记忆系统提供了"写入时 vs 读取时裁剪"的完整权衡图谱。
