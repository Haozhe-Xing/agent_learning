...
### [Agent 开口前：多 Agent 系统的事前失败风险推断（2026）](https://arxiv.org/abs/2607.26836)

**发表**：2026 年 7 月 30 日 | [arXiv:2607.26836](https://arxiv.org/abs/2607.26836)

**核心贡献**：多 Agent 系统中，单个 Agent 的幻觉不仅影响自身输出，还会沿着 Agent 协作链向下游传播，放大并固化错误——这是多 Agent 系统不同于单 Agent 系统的核心可靠性挑战。本文提出**事前失败风险推断框架（pre-hoc failure risk inference）**：在 Agent 开始生成回复之前，基于当前任务上下文、Agent 角色和协作图结构，预测该 Agent 在当前步产生幻觉的风险，并在高风险情况下触发额外验证或任务重分配。框架区分两类风险：单 Agent 内生幻觉风险（基于查询类型与模型置信度）和跨 Agent 传播风险（基于上游输出质量与依赖深度）。在多 Agent 模拟环境中，事前干预相比事后校正将最终任务错误率降低 23%，同时减少了不必要的重新验证调用。

**与本章关系**：直接对应本章「多 Agent 系统可靠性」与「多 Agent 协作机制」知识点，将多 Agent 系统的质量保障从"事后检错"升级为"事前预判"，是继 SafeFlow（信息流安全）之后在可靠性维度为多 Agent 系统提供主动保障的最新研究方向，对构建生产级高可靠多 Agent 系统具有直接架构参考价值。
