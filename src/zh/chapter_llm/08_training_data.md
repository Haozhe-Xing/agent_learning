...
> 🔗 **与第11章的分工说明**  
> 本节（3.8）聚焦于**数据工程视角**：如何准备高质量的 SFT 和 RL 训练数据，包括数据格式规范、质量标准和常见陷阱。  
> **第11章 Agentic-RL**（第11.2节起）则深入**算法视角**：LoRA 参数高效微调原理、PPO/DPO/GRPO 强化学习算法和完整训练 Pipeline。  
> 两章组合阅读效果最佳。

*上一节：[3.7 基础模型架构深度解析](./07_model_architecture.md)*
