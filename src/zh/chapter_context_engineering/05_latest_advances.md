...
### [C-DIC：多轮对话的上下文驱动增量压缩](https://arxiv.org/abs/2606.12411)

**发表**：2026 年 6 月 10 日 | ICML 2026 | [arXiv:2606.12411](https://arxiv.org/abs/2606.12411)

**核心贡献**：随多轮对话历史增长，简单截断或整体摘要均会损失信息。C-DIC 将对话视为多条交织的主题线索（Contextual Threads），为每条线索维护可修订的压缩状态，存储于单一紧凑对话记忆中，每轮执行轻量级"检索-修订-写回"循环，跨轮次共享信息并更新过时记忆，适配截断时间反向传播（TBPTT）实现无完整历史反向传播的多轮训练。在长对话基准上表现优于现有方法，在数百轮对话中保持稳定推理延迟和困惑度，已被 ICML 2026 接收。

**与本章关系**：对应本章"上下文压缩与长期记忆管理"知识点，将"压缩"从静态一次性操作升级为动态、可修订的增量过程，直接解决了多轮 Agent 对话中上下文随对话轮次无限增长的核心挑战，是 Context Engineering 在多轮场景下的重要方法论补充。

---

### [CWL：面向长视野 Agent 的结构化上下文驱逐方案](https://arxiv.org/abs/2606.11213)

**发表**：2026 年 6 月 11 日 | [arXiv:2606.11213](https://arxiv.org/abs/2606.11213)

**核心贡献**：长视野 Agent 在处理数百个顺序任务时，上下文窗口面临溢出压力。现有方案（摘要压缩、最近截断）都有不可预测的信息丢失或语义盲目性。CWL（Context Window Lifecycle）提出**结构化上下文驱逐**：Agent 在执行过程中将轨迹注释为有类型、有依赖关系的"情节片段"（Episodes），构成情节依赖图；当 token 预算超限时，无需 LLM 调用的确定性策略按优先级驱逐"效果已在环境中持久化"的动作片段，保留用户轮次和当前活跃推理上下文，使活动上下文维持在稳定上限。在长视野基准上，单一 Agent 会话处理 8000 万 token、完成 89 个顺序任务，任务准确率无可测量下降。
