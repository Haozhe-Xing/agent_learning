...
- **最优信息输入**：不是"越多越好"，也不是"越少越好"，而是在有限预算内选出**最优子集**。往上下文里塞太多无关信息，反而会降低 LLM 的表现（我们将在 7.2 节详细讨论这个违反直觉的现象）。
...
在后续的 7.2 节中，我们会更详细地讨论 LLM 的注意力分布机制，那时你会从技术层面理解"为什么少反而更好"。
...
> 💡 **实践中的动态裁剪**：Anthropic 的 Claude Agent 在执行长任务时，会自动触发"compaction"（压缩整合）——当上下文接近窗口限制时，将历史对话压缩为精炼摘要，释放空间给新的工具结果和用户交互。这种"压力触发式"的动态裁剪，让 Agent 能在有限的窗口内持续高效工作。我们将在 7.3 节详细讲解这种策略的原理和实现。
...
*下一节：[7.2 上下文窗口管理与注意力预算](./02_context_window.md)*
