...
> **这一步解决什么问题**：工具是 Agent 的"手"。在写 Agent 主循环之前，先把"能做什么"定义清楚——每个工具要有 `name`（标识）、`description`（给 LLM 读的说明）和 `run`（实际执行）。`description` 写得好坏直接决定 LLM 会不会在正确时机调用它。
...
> **这一步解决什么问题**：主循环是 Agent 的"大脑"——它反复做三件事：**把工具描述和上下文喂给 LLM → 解析 LLM 的回复 → 是工具调用就执行并回写结果，是最终回答就返回**。这个"感知-思考-行动"循环就是第 1 章讲的核心，现在看它落地成代码。
