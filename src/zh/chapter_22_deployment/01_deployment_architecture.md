...
传统服务的日志是线性的（请求 → 处理 → 响应），而 Agent 的执行是树状或图状的（推理 → 工具 A → 子推理 → 工具 B → 回溯 → 工具 C → 最终回复）。标准的日志和监控工具难以捕捉这种复杂的执行路径。
...
>
> 各服务分别连接：Redis / Vector DB / External APIs
...
![Agent 生产部署架构](../svg/chapter_22_deployment_01_architecture.svg)
...
> 💡 **延伸阅读**：关于模型路由评估和 A/B 测试，详见 [18.7 A/B 测试与回归测试自动化](../chapter_20_evaluation/07_ab_testing.md) 和 [18.8 模型路由评估](../chapter_20_evaluation/08_model_routing.md)。

> **下一节预告**：接下来我们用 FastAPI 把 Agent 封装成一个完整的 API 服务。

---

[22.2 API 服务化：FastAPI / Flask 封装](./02_api_service.md)
