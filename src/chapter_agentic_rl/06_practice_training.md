# 18.6 实战：完整 Agentic-RL 训练 Pipeline
...
| **18.3 PPO** | 策略梯度、重要性采样、优势函数、Clip 机制 | PPO 是 RLHF 的经典算法，但 Critic 导致显存占用 ≈ 3× |
| **18.4 DPO** | 隐式奖励、Bradley-Terry 模型、闭式解 | DPO 将 RL 转化为监督学习，极简但无法在线探索 |
| **18.5 GRPO + 奖励设计** | 组内比较、多维度奖励、奖励黑客防御 | GRPO 将显存需求从 3× 降至 1.5×；奖励函数是 RL 训练效果的决定性因素 |
| **18.6 实战** | 完整 Pipeline、评估对比 | GSM8K 上：基座 ~40% → SFT ~50% → GRPO ~60% |
