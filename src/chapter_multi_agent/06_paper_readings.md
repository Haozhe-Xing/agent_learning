...
关键特性：
1. 模块化设计：每个 Agent 可以独立替换或升级
2. 模型无关：支持 GPT-4o、Claude 等不同 LLM
3. 动态重规划：Orchestrator 在执行中持续评估进度
4. 自我恢复：当子任务失败时自动调整策略
```
...
```
AutoGPT / BabyAGI (2023初，单 Agent 自主循环)
    ↓
ChatDev (聊天驱动)  ←→  MetaGPT (SOP 驱动)
    ↓                       ↓
AutoGen (灵活对话)        AgentVerse (涌现行为)
    ↓                       ↓
Swarm (极简交接)         Magentic-One (通用系统)
    ↓                       ↓
OpenAI Agents SDK        AutoGen 0.4 事件驱动
    ↓                       ↓
    └───── A2A + MCP (2025，标准化 Agent 互操作) ─────┘
```
