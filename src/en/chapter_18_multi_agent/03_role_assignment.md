# 18.3 Role Assignment and Task Allocation
...
![Specialized Agent role assignment architecture](../svg/chapter_18_multi_agent_03_roles.svg)
...
The `DynamicTaskAllocator` above only solves "**who can do it**" (capability matching), not "**who is free right now**." When multiple homogeneous Agents can all handle the same kind of task (say 5 "code reviewers"), always assigning to the first one creates "some Agents overworked, others idle." **Load balancing** solves exactly this — spreading tasks evenly across available Agents.
...
class LoadBalancer:
    """Multi-Agent load balancer: pick the least-busy Agent from a homogeneous pool"""

    def __init__(self, agents: list[str]):
        # Each Agent has a pending queue; queue length = current load
        self.queues: dict[str, deque] = {a: deque() for a in agents}
        self._rr = itertools.cycle(agents)   # round-robin cursor

    # Strategy 1: Round-Robin — assign in turn, the simplest
    def round_robin(self, task) -> str:
        agent = next(self._rr)
        self.queues[agent].append(task)
        return agent

    # Strategy 2: Least-Load — assign to the Agent with the shortest queue
    def least_load(self, task) -> str:
        agent = min(self.queues, key=lambda a: len(self.queues[a]))
        self.queues[agent].append(task)
        return agent

    # Strategy 3: Capability-Aware — combine "bidding" (contract net, see 18.2):
    # Agents self-report "current load + completion confidence", pick the best
    def capability_aware(self, task, bids: dict[str, tuple[float, float]]) -> str:
        # bids: {agent: (estimated_cost, confidence)}; low-confidence agents dropped
        valid = {a: c for a, (c, conf) in bids.items() if conf > 0.5}
        if not valid:
            return self.least_load(task)     # no bidders -> degrade to least-load
        # score = estimated cost + current queue length penalty, take the minimum
        agent = min(valid, key=lambda a: valid[a] + len(self.queues[a]))
        self.queues[agent].append(task)
        return agent

    def finish(self, agent: str):
        """An Agent finished a task, dequeue (load decreases)"""
        if self.queues[agent]:
            self.queues[agent].popleft()
```

> 💡 **Selection intuition**: homogeneous Agents with equal speed → **round-robin** is simplest; large speed variance → **least-load**; Agents differ in capability and self-report dynamically → **capability-aware** (closest to real production, but also the most complex).

### Backpressure: Preventing Avalanches

Load balancing only "spreads evenly," it can't "accept unlimited work." When every Agent's queue is full, pushing more tasks in only collapses the whole system — this is **backpressure** from distributed systems: upstream must sense downstream saturation and proactively slow down or reject.

```python
MAX_QUEUE = 10   # per-Agent queue cap

def enqueue_with_backpressure(lb: LoadBalancer, task) -> str:
    """Enqueue with backpressure: reject when all full, instead of forcing in"""
    if all(len(q) >= MAX_QUEUE for q in lb.queues.values()):
        # All full: reject and let upstream retry later (rather than queueing
        # unboundedly and draining memory/latency)
        raise RuntimeError("All Agent queues are full, please retry later")
    return lb.least_load(task)
```

> 🔴 **Most common incident**: a Supervisor fans out sub-agents without limit (e.g. "search 500 web pages" spawns 500), instantly blowing up LLM rate limits and memory — this is the so-called **fork-bomb**. In engineering you must put a hard cap on "**concurrent sub-agent count**" (e.g. `Semaphore(8)`), queueing excess tasks instead of unbounded spawning.
...
The `ResilientTeam` above handles "**degradation after a task fails**" (switching to a backup Agent). But in multi-Agent **distributed execution**, five categories of "system-level" faults must be handled separately — they are not "business failures" but "Agent process/call-level" anomalies:

| Mechanism | Solves | Core approach |
|------|------|---------|
| **Timeout** | Agent hangs (LLM unresponsive) | Set a `timeout` per call; interrupt and degrade on expiry |
| **Bounded retry** | Transient failures (network jitter, rate limits) | Retry on failure, but with a **cap + backoff** to avoid retry storms |
| **Circuit breaker** | One Agent failing continuously drags down the whole | After N consecutive failures → temporarily break that Agent, fail fast |
| **Heartbeat/watchdog** | Agent goes silent (process froze without erroring) | Periodic heartbeat; no response past timeout → mark dead and restart |
| **Idempotency** | Retries cause duplicate side effects | Tasks carry an idempotency key; repeated submission executes once |

```python
import time

class SystemResilientAgent:
    """Agent wrapper with system-level resilience: timeout + bounded retry + circuit breaker + idempotency"""
    ...
    def __init__(self, agent, timeout: float = 30.0, max_retries: int = 3,
        ...
        self._fail_streak = 0            # consecutive failure count (for breaker)
        self._seen_ids: set[str] = set() # idempotency key set

    def call(self, task_id: str, task: str) -> str:
        # Idempotency: same task_id re-submitted returns cached, no duplicate side effects
        if task_id in self._seen_ids:
            return "[idempotent hit] task already executed, skipped"

        if self._fail_streak >= self.breaker_threshold:
            # Circuit breaker: too many consecutive failures, fail fast instead of
            # hammering a known-broken Agent
            return "[circuit open] Agent failing repeatedly, temporarily paused"

        for attempt in range(self.max_retries):
            try:
                # Timeout: use a thread pool to "give up at deadline", preventing a
                # hung Agent from stalling the whole pipeline
                import concurrent.futures
                with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
                    result = ex.submit(self.agent.process, task).result(timeout=self.timeout)
                self._fail_streak = 0          # success resets the streak
                self._seen_ids.add(task_id)     # record idempotency key
                return result
            except Exception:
                self._fail_streak += 1
                # Bounded retry + exponential backoff: wait 2^N seconds after the Nth
                # failure, avoiding retry storms
                if attempt < self.max_retries - 1:
                    time.sleep(2 ** attempt)
        return "[failed] retries exhausted, task failed"
```

> 🔑 **Key principle**: retries **must be idempotent** — if a "send email" Agent fails and retrying sends a second email, the retry itself becomes the disaster. The four system-level mechanisms (timeout/retry/breaker/heartbeat) are "infrastructure"; idempotency is a "business contract" — both are indispensable.
...
Two levels of task allocation:
- **By capability** (`DynamicTaskAllocator`): the LLM judges "who can do it"
- **By load balancing** (`LoadBalancer`): spread evenly across a homogeneous pool — round-robin / least-load / capability-aware, plus **backpressure** to prevent avalanches

System-level fault tolerance (distinct from business degradation):
- **Timeout** prevents hangs, **bounded retry** prevents retry storms, **circuit breaker** prevents dragging down the whole, **heartbeat** prevents silent death, **idempotency** keeps retries safe
- Especially guard against **fork-bombs**: hard-cap the concurrent sub-agent count

> 💡 **Further reading**: For an in-depth comparison of the Supervisor pattern and the decentralized pattern, see [18.4 Supervisor Mode vs. Decentralized Mode](./04_supervisor_vs_decentralized.md).

---

*Next section: [18.4 Supervisor Mode vs. Decentralized Mode](./04_supervisor_vs_decentralized.md)*
