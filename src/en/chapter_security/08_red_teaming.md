...
![Traditional security testing vs. AI red teaming](../svg/chapter_security_08_redteam_compare.svg)
...
![Agent cascading-risk example](../svg/chapter_security_08_cascade_risk.svg)
...
![Gradual information leakage attack](../svg/chapter_security_08_info_leak.svg)
...
![Resource exhaustion attack example](../svg/chapter_security_08_resource_exhaust.svg)
...
├────────────────────────────────────────────────────────────────────────┤
│                                                                        │
│  1. Attack surface mapping ── identify all entry points and permissions│
│       ↓                                                                │
│  2. Attack tree modeling   ── systematically model all attack paths    │
│       ↓                                                                │
│  3. Scenario library       ── derive concrete test cases from the tree │
│       ↓                                                                │
│  4. Automated execution    ── run the scenarios automatically          │
│       ↓                                                                │
│  5. Result evaluation      ── measure attack success rate and impact   │
│       ↓                                                                │
│  6. Fix and verify         ── patch the holes, then regression-test    │
│                                                                        │
└────────────────────────────────────────────────────────────────────────┘
```
...
> 💡 **Connection to 17.7 A/B testing**: The idea behind security regression testing follows directly from [18.7 A/B Testing and Regression Test Automation](../chapter_evaluation/07_ab_testing.md). It is a good practice to fold your security test cases into the CI/CD regression suite so they run automatically before every deployment, ensuring that safety-policy updates never introduce a security regression.
...
> 📖 **Want to dig into the academic frontier of Agent safety evaluation?** Read [19.6 Paper Readings: Frontiers in Security and Reliability Research](./06_paper_readings.md), where AgentDojo, InjecAgent, and ASB provide standardized frameworks for evaluating Agent security.
>
> 💡 **Connection to Chapter 18**: Red-teaming results should feed into the framework of [18.7 A/B Testing and Regression Test Automation](../chapter_evaluation/07_ab_testing.md) — make your security test cases part of the regression suite so that safety is automatically verified before every deployment.
>
> 💡 **Connection to Chapter 23**: Computer Use Agents ([23.5 Computer Use and GUI Agents](../chapter_multimodal/05_computer_use_agent.md)) hold operating-system-level permissions, so red teaming must pay special attention to the safety boundaries of GUI actions — for example, preventing the Agent from performing dangerous operations in the desktop environment.
>
> 💡 **Connection to the previous section**: [19.7 Guardrails Runtime Protection](./07_guardrails_runtime.md) is the defense, and this section's red teaming is the validation — together they form a **"defend-and-verify" loop** in which neither half can be skipped.
...
[19.7 Guardrails Runtime Protection](./07_guardrails_runtime.md)
