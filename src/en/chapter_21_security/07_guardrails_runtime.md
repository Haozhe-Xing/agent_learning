# 21.7 Guardrails Runtime Protection
...
![Trust chain gaps when relying on prompts alone](../svg/chapter_21_security_07_prompt_trust_chain.svg)
...
![The trust chain strengthened by Guardrails](../svg/chapter_21_security_07_guardrails_chain.svg)
...
![Three-layer guardrails architecture](../svg/chapter_21_security_07_three_layers.svg)
...
![The Validator chained validation framework](../svg/chapter_21_security_07_validator_chain.svg)
...
![The dual-layer Guardrails filtering mechanism](../svg/chapter_21_security_07_guardrails_chain.svg)
...
![Guardrails performance impact analysis](../svg/chapter_21_security_07_guardrails_perf.svg)
...
> 📖 **Want to go deeper into the research frontier of runtime protection?** Read [21.6 Paper Readings: Frontier Research in Security and Reliability](./06_paper_readings.md), where SafeAgent [1] presents the latest approach to upgrading Guardrails into a stateful decision architecture.
>
> 💡 **Connection to Chapter 18**: the effectiveness of guardrails needs to be verified through systematic evaluation — which is exactly the use case for [18.7 A/B Testing and Regression Test Automation](../chapter_20_evaluation/07_ab_testing.md). It is a good idea to write regression tests for every guardrails rule so that rule updates never introduce a safety regression.
>
> 💡 **Connection to Chapter 23**: a Computer Use Agent ([23.5 Computer Use and GUI Agents](../chapter_25_multimodal/05_computer_use_agent.md)) has far more privileges than a chat Agent — it can operate the desktop and the browser — so its guardrails requirements are higher. Strict permission control and security auditing must be enforced at the tool-call layer.
...
[21.8 Red Teaming Methodology](./08_red_teaming.md)
