...
输入查询 q
    ↓
检索模型（DPR，Dense Passage Retriever）
  → 从知识库中检索 top-k 相关文档 {z1, z2, ..., zk}
    ↓
生成模型（BART）
  → 基于查询 q 和检索到的文档生成回答
...
│   - 查询模块（改写、分解、路由）                               │
│   - 检索模块（稠密/稀疏/混合检索）                             │
│   - 重排序模块（Cross-Encoder、LLM Reranker）                │
│   - 生成模块（不同的 Prompt 策略）                             │
│   - 验证模块（事实核查、幻觉检测）                             │
│   像乐高积木一样自由组合                                      │
└─────────────────────────────────────────────────────────────┘
...
RAG (2020，基础范式)
    ↓
Self-RAG (2023，自适应检索决策)
    ↓
CRAG (2024，检索质量纠错)          GraphRAG (2024，结构化知识)
    ↓                                ↓
Modular RAG (2024，模块化框架)     LightRAG (2024，轻量级图 RAG)
    ↓                                ↓
    └──────── Agentic RAG (2025) ────┘
              Agent 主导的智能检索-推理循环
```
