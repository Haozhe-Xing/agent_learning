...
```
1. 记忆流（Memory Stream）
   ├── 记录 Agent 的所有经历（观察、对话、行动）
   ├── 每条记忆附带时间戳、重要性评分
   └── 是记忆系统的"原始数据层"

2. 反思（Reflection）
   ├── 定期（当积累的记忆重要性超过阈值时）触发
   ├── 从近期记忆中提炼高层次的洞察
   │   例如：多次观察到"Klaus在画画" → 反思："Klaus对艺术充满热情"
   └── 反思结果也存入记忆流（可以对反思再反思，形成层次化理解）

3. 检索（Retrieval）
   ├── 当 Agent 需要做决策时，从记忆流中检索相关记忆
   └── 检索基于三个维度的加权组合：
       - 时效性（Recency）：越新的记忆权重越高（指数衰减）
       - 重要性（Importance）：越重要的记忆权重越高（1-10评分）
       - 相关性（Relevance）：与当前情境越相关权重越高（嵌入向量相似度）
```
...
```
┌─────────────────────────────────────────────────────────┐
│  计算机操作系统                    LLM Agent (MemGPT)    │
├─────────────────────────────────────────────────────────┤
│  CPU 寄存器 ←→ LLM 的注意力焦点                         │
│  主内存 (RAM) ←→ 上下文窗口 (Context Window)            │
│  硬盘/SSD ←→ 外部存储 (数据库/文件)                     │
│  缺页中断 ←→ 信息不在上下文中，需要"换入"                │
│  页面置换算法 ←→ 记忆的换入换出策略                      │
│  虚拟内存 ←→ "无限"记忆的幻象                           │
